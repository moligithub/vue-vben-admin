...
export function getDownloadUrl(type: string, fileName: string = '') {
  return defHttp.get({ url: `/bot/filehandler/Download/${type}/${fileName}` });
}
// 获取全部下载文件链接（打包下载）
export function getPackDownloadUrl(type: string, data) {
  return defHttp.post({ url: `/bot/filehandler/PackDownload/${type}`, data });
}
// 预览文件
...
export function getTemplateDownload(url) {
  return defHttp.get({ url: `/bot/${url}/TemplateDownload` });
}
// 下载导入示例模板
export function getImportPreview(url, data) {
  return defHttp.get({ url: `/bot/${url}/ImportPreview`, data });
}
// 导入数据
export function importData(url, data) {
  return defHttp.post({ url: `/bot/${url}/ImportData`, data });
}
// 导入数据
export function getImportExceptionData(url, data) {
  return defHttp.post({ url: `/bot/${url}/ImportExceptionData`, data });
}
// 查询附近数据
