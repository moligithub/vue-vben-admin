          ...
          <div class="parameter-box">
            <div class="left-pane">
              <div class="left-pane-list">
                  ...
                  <BasicTable @register="registerTable" :data-source="parameterList">
                    <template #bodyCell="{ column, record, index }">
                      ...
                      <template v-if="column.key === 'action' && !getIsSystem(record.field)">
                        <a-space :size="10">
                          <i class="icon-bts icon-bts-btn-edit" @click="addOrUpdateParameter(record)"></i>
                          <i class="icon-bts icon-bts-delete" @click="removeParameter(index)"></i>
                        </a-space>
                      </template>
                    </template>
                  </BasicTable>
                ...
                <div class="table-add-action" @click="addOrUpdateParameter()" v-if="dataForm.messageSource === '2'">
                  <a-button type="link" preIcon="icon-bts icon-bts-btn-add">{{ t('common.addText') }}</a-button>
                </div>
              </div>
            </div>
            <div class="right-pane" v-if="dataForm.messageType != 3 && dataForm.messageType != 7">
              ...
              <a-form-item name="content" v-if="dataForm.messageType == 2">
                ...
                <TsaEditorvider v-model:value="dataForm.content" />
              </a-form-item>
              ...
              <a-row :gutter="20">
                ...
                <a-col :span="12" v-if="dataForm.messageType == 7">
                  <a-form-item label="跳转方式" name="wxSkip">
                    <tsa-select v-model:value="dataForm.wxSkip" placeholder="请选择" :options="wxSkipList" />
                  </a-form-item>
                </a-col>
                  ...
                  <a-table :data-source="smsList" :columns="getSmsParameterColumns" size="small" :pagination="false">
                    ...
                    <template #bodyCell="{ column, record, index }">
                      ...
                      <template v-if="column.key === 'action'">
                        <i class="icon-bts icon-bts-delete" danger @click="removeSmsData(index)"></i>
                      </template>
                    </template>
                  </a-table>
                </div>
                <div class="table-add-action" @click="addSmsData">
                  <a-button type="link" preIcon="icon-bts icon-bts-btn-add">添加一行</a-button>
                </div>
              </div>
            </div>
          </div>
...
<script lang="ts" setup>
  ...
  import { getInfo, create, update } from '/@/api/imcenter/msgTemplate';
