<template>
  <div class="tsa-content-wrapper document-wrapper bg-white">
    <a-tabs v-model:activeKey="activeKey" tab-position="left" class="common-left-tabs">
      <a-tab-pane key="all">
        <template #tab><i class="icon-bts icon-bts-extend-folder-open"></i>全部文档</template>
        <div class="container">
          ...
          <div class="tsa-common-search-box">
            ...
            <div class="tsa-common-search-box-right">
              ...
              <a-button type="primary" preIcon="icon-bts icon-bts-btn-upload" @click="uploadFile()" class="ml-10px">上传文件</a-button>
            </div>
          </div>
            ...
            <template #bodyCell="{ column, record }">
              <template v-if="column.key === 'fullName'">
                <span v-if="record.type">
                  <i :class="'icon-bts ' + toFileExt(record.fileExtension) + ' i-default'" />
                  {{ record.fullName }}
                </span>
                <span class="cursor-pointer" v-else @click="openFolder(record)">
                  <i class="icon-bts icon-bts-extend-folder text-warning" />
                  {{ record.fullName }}
                </span>
              </template>
              <template v-if="column.key === 'isShare'">
                <span v-if="record.type && record.isShare" title="共享文件"><i class="icon-bts icon-bts-extend-thumbs-up i-default"></i></span>
              </template>
      ...
      <a-tab-pane key="shareOut">
        <template #tab><i class="icon-bts icon-bts-extend-thumbs-up"></i>我的共享</template>
        <div class="container">
          <Breadcrumb class="!mb-10px">
      ...
      <a-tab-pane key="shareTome">
        <template #tab><i class="icon-bts icon-bts-extend-share"></i>共享给我</template>
        <div class="container">
          <Breadcrumb class="!mb-10px">
      ...
      <a-tab-pane key="trash">
        <template #tab><i class="icon-bts icon-bts-extend-trash"></i>回收站</template>
        <div class="container">
          <Breadcrumb class="!mb-10px">
...
<script lang="ts" setup>
  ...
  import { toFileSize } from '/@/utils/tsa';
  ...
  function toFileExt(ext) {
    ...
    if (ext == 'doc' || ext == 'docx') {
      return 'icon-bts-file-word';
    }
    if (ext == 'xls' || ext == 'xlsx') {
      return 'icon-bts-file-excel';
    }
    if (ext == 'ppt' || ext == 'pptx') {
      return 'icon-bts-file-ppt';
    }
    if (ext == 'rar' || ext == 'zip') {
      return 'icon-bts-file-zip';
    }
    if (ext == 'txt' || ext == 'log') {
      return 'icon-bts-file-text';
    }
    if (ext == 'html' || ext == 'cs' || ext == 'xml') {
      return 'icon-bts-file-code';
    }
    if (ext == 'gif' || ext == 'jpg' || ext == 'jpeg' || ext == 'bmp' || ext == 'png') {
      return 'icon-bts-file-image1';
    }
    if (ext == 'pdf') {
      return 'icon-bts-file-pdf';
    }
    if (ext == 'mp3') {
      return 'icon-bts-file-audio';
    }
    if (ext == 'mp4') {
      return 'icon-bts-file-movie';
    }
    return 'icon-bts-file-blank';
  }
