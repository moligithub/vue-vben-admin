<template>
  <BasicModal v-bind="$attrs" @register="registerModal" title="日志详情" width="800px" :footer="null" class="tsa-log-detail-modal">
      ...
      <div class="left-board">
        <div class="item-box" :class="{ active: activeItem.id == item.id }" v-for="(item, index) in list" @click="handleClick(item, index)">
          <div class="top">
            ...
            <i class="icon icon-bts icon-bts-success" v-if="item.resultType" />
            <i class="icon icon-bts icon-bts-fail" v-else />
          </div>
          <div class="bottom">
            <i class="icon-bts icon-bts-clock pr-4px" v-if="item.type == 1" />
            <i class="icon-bts icon-bts-btn-refresh pr-4px" v-else />
...
<script lang="ts" setup>
  import { getTaskInfo, nodeRetryTask } from '/@/api/lowcodedev/integrate';
