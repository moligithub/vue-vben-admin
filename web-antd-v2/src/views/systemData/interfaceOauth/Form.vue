      ...
      <a-col :span="14" :offset="5">
        <BasicForm @register="registerForm" :disabled="state.isDetail">
          ...
          <template #verifySignature="{ model, field }">
            <tsa-switch v-model:value="model[field]" :disabled="state.isDetail" />
  ...
  function init(data) {
    ...
    if (state.id) {
      ...
      getInfo(state.id).then(res => {
        ...
        if (data.list) {
          data.list.map(item => {
            item.url = `${globSetting.apiUrl}/bot/systemmgr/DataInterface/${item.id}/Actions/Response` + (tenantId ? '?tenantId=' + tenantId : '');
