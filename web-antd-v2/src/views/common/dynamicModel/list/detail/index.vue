...
<script lang="ts" setup>
  import { getDataChange, getConfigData } from '/@/api/lowcodedev/visualDev';
  ...
  function fillFormData(form, data) {
    const loop = (list, parent?) => {
      for (let i = 0; i < list.length; i++) {
        let item = list[i];
        if (item.__vModel__) {
          if (item.__config__.tsaDK === 'relationForm' || item.__config__.tsaDK === 'popupSelect') {
          ...
          if (['relationFormAttr', 'popupAttr'].includes(item.__config__.tsaDK)) {
            item.__config__.defaultValue = data[item.relationField.split('_tsaTable_')[0] + '_' + item.showField];
          }
        }
