<template>
  <a-col
    ...
    <template v-if="getConfig.layout === 'colFormItem'">
      <template v-if="getConfig.tsaDK === 'divider'">
        <tsa-divider :contentPosition="item.contentPosition" :content="item.content" />
      </template>
      <template v-else>
        <a-form-item :name="item.__vModel__" :labelCol="getLabelCol">
          ...
          <template v-if="getConfig.tsaDK === 'text'">
            <tsa-text :content="item.content" :textStyle="item.textStyle" />
          </template>
          <template v-else-if="getConfig.tsaDK === 'link'">
            <tsa-link :content="item.content" :href="item.href" :target="item.target" :textStyle="item.textStyle" />
          </template>
          <template v-else-if="getConfig.tsaDK === 'alert'">
            <tsa-alert
              :title="item.title"
              :type="item.type"
          ...
          <template v-else-if="getConfig.tsaDK === 'groupTitle'">
            <tsa-group-title :content="item.content" :contentPosition="item.contentPosition" :helpMessage="item.helpMessage" />
          </template>
          <template v-else-if="getConfig.tsaDK === 'button'">
            <tsa-button :align="item.align" :buttonText="item.buttonText" :type="item.type" :disabled="item.disabled" />
          </template>
          <template v-else-if="getConfig.tsaDK === 'uploadFile'">
            <tsa-upload-file v-model:value="getConfig.defaultValue" detailed disabled />
          </template>
          <template v-else-if="getConfig.tsaDK === 'uploadImg'">
            <tsa-upload-img v-model:value="getConfig.defaultValue" detailed disabled />
          </template>
          <template v-else-if="getConfig.tsaDK === 'colorPicker'">
            <tsa-color-picker v-model:value="getConfig.defaultValue" :showAlpha="item.showAlpha" :colorFormat="item.colorFormat" disabled />
          </template>
          <template v-else-if="getConfig.tsaDK === 'rate'">
            <tsa-rate v-model:value="getConfig.defaultValue" :count="item.count" :allowHalf="item.allowHalf" disabled />
          </template>
          <template v-else-if="getConfig.tsaDK === 'slider'">
            <tsa-slider v-model:value="getConfig.defaultValue" :min="item.min" :max="item.max" :step="item.step" disabled />
          </template>
          <template v-else-if="getConfig.tsaDK === 'editor'">
            <div v-html="getConfig.defaultValue"></div>
          </template>
          <template v-else-if="getConfig.tsaDK === 'relationForm'">
            <p class="link-text" @click="toDetail(item)">{{ item.name }}</p>
          </template>
          <template v-else-if="getConfig.tsaDK === 'popupSelect'">
            <p>{{ item.name }}</p>
          </template>
          <template v-else-if="getConfig.tsaDK === 'barcode'">
            <tsa-barcode
              :format="item.format"
          ...
          <template v-else-if="getConfig.tsaDK === 'qrcode'">
            <tsa-qrcode
              :format="item.format"
          ...
          <template v-else-if="getConfig.tsaDK === 'inputNumber'">
            <tsa-input-number
          ...
          <template v-else-if="getConfig.tsaDK === 'calculate'">
            <tsa-calculate
          ...
          <template v-else-if="getConfig.tsaDK === 'location'">
            <tsa-location v-model:value="getConfig.defaultValue" :enableLocationScope="item.enableLocationScope" detailed />
          </template>
          <template v-else-if="getConfig.tsaDK === 'sign'">
            <tsa-sign v-model:value="getConfig.defaultValue" detailed />
          </template>
          <template v-else-if="getConfig.tsaDK === 'signature'">
            <tsa-signature v-model:value="getConfig.defaultValue" detailed />
          </template>
          <template v-else-if="getConfig.tsaDK === 'iframe'">
            <tsa-iframe
              :href="item.href"
              :height="item.height"
          ...
          <template v-else-if="getConfig.tsaDK === 'input'">
            <tsa-input
              v-model:value="getConfig.defaultValue"
              :addonBefore="item.addonBefore"
              :addonAfter="item.addonAfter"
    ...
    <template v-else>
      <template v-if="getConfig.tsaDK === 'card'">
      ...
      <a-row v-if="getConfig.tsaDK === 'row'">
        <Item v-for="(childItem, childIndex) in getConfig.children" v-bind="getBindValue" :key="childIndex" :item="childItem" @toDetail="toDetail" />
      </a-row>
      <template v-if="getConfig.tsaDK === 'tab'">
      ...
      <template v-if="getConfig.tsaDK === 'collapse'">
      ...
      <template v-if="getConfig.tsaDK === 'table'">
        <a-form-item v-if="!getConfig.noShow">
          <TsaGroupTitle :content="getConfig.label" v-if="getConfig.showTitle && getConfig.label" :helpMessage="getConfig.tipLabel" :bordered="false" />
          <a-table
            ...
            <template #bodyCell="{ column, record, index }">
              ...
              <template v-else-if="column.__config__?.tsaDK === 'uploadFile'">
                <tsa-upload-file v-model:value="record[column.dataIndex]" detailed disabled />
              </template>
              <template v-else-if="column.__config__?.tsaDK === 'uploadImg'">
                <tsa-upload-img v-model:value="record[column.dataIndex]" detailed disabled />
              </template>
              <template v-else-if="column.__config__?.tsaDK === 'colorPicker'">
                <tsa-color-picker v-model:value="record[column.dataIndex]" :showAlpha="column.showAlpha" :colorFormat="column.colorFormat" disabled />
              </template>
              <template v-else-if="column.__config__?.tsaDK === 'rate'">
                <tsa-rate v-model:value="record[column.dataIndex]" :count="column.count" :allowHalf="column.allowHalf" disabled />
              </template>
              <template v-else-if="column.__config__?.tsaDK === 'slider'">
                <tsa-slider v-model:value="record[column.dataIndex]" :min="column.min" :max="column.max" :step="column.step" disabled />
              </template>
              <template v-else-if="column.__config__?.tsaDK === 'relationForm'">
                <p class="link-text" @click="toTableDetail(column, record[column.dataIndex + '_id'])">{{ record[column.dataIndex] }}</p>
              </template>
              <template v-else-if="['relationFormAttr', 'popupAttr'].includes(column.__config__?.tsaDK)">
                <p v-if="!record[column.dataIndex]">{{ record[column.relationField.split('_tsaTable_')[0] + '_' + column.showField] }}</p>
                <p v-else>{{ record[column.dataIndex] }}</p>
              </template>
              <template v-else-if="column.__config__?.tsaDK === 'inputNumber'">
                <tsa-input-number
              ...
              <template v-else-if="column.__config__?.tsaDK === 'calculate'">
                <tsa-calculate
                  :rowIndex="index"
              ...
              <template v-else-if="column.__config__?.tsaDK === 'location'">
                <tsa-location v-model:value="record[column.dataIndex]" :enableLocationScope="column.enableLocationScope" detailed />
              </template>
              <template v-else-if="column.__config__?.tsaDK === 'sign'">
                <tsa-sign v-model:value="record[column.dataIndex]" detailed />
              </template>
              <template v-else-if="column.__config__?.tsaDK === 'signature'">
                <tsa-signature v-model:value="record[column.dataIndex]" detailed />
              </template>
              <template v-else-if="column.__config__?.tsaDK === 'input'">
                <tsa-input
                  v-model:value="record[column.dataIndex]"
                  :addonBefore="column.addonBefore"
                  :addonAfter="column.addonAfter"
      ...
      <template v-if="getConfig.tsaDK === 'tableGrid'">
        <table
...
<script lang="ts" setup>
  ...
  import { thousandsFormat } from '/@/utils/tsa';
  ...
  const getColumns = computed(() => {
    if (unref(getConfig).tsaDK !== 'table') return [];
