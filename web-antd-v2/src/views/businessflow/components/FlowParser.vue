...
<script lang="ts" setup>
  ...
  import { create, update } from '/@/api/businessflow/workFlowForm';
  import { revoke, press } from '/@/api/businessflow/flowLaunch';
      ...
      .then(res => {
        ...
        const formUrl =
          state.flowFormInfo.formType == 2
            ? 'businessflow/workFlowForm/dynamicForm'
            : state.flowFormInfo.urlAddress
            ? state.flowFormInfo.urlAddress.replace(/\s*/g, '')
            : `businessflow/workFlowForm/${state.flowFormInfo.enCode}`;
