    ...
    <a-form
      ...
      <template v-if="eventType === 'freeApprover'">
        <a-form-item label="加签人员" name="freeApproverUserId" :rules="[{ required: true, message: `必填`, trigger: 'change' }]">
          <tsa-user-select v-model:value="dataForm.freeApproverUserId" placeholder="请选择" allowClear />
        </a-form-item>
      ...
      <template v-if="eventType === 'audit' || (eventType === 'freeApprover' && dataForm.freeApproverType === 2)">
        <a-form-item
          ...
          <tsa-select v-model:value="dataForm.branchList" multiple placeholder="请选择" allowClear @change="onBranchChange" :options="branchList" showSearch />
        </a-form-item>
        <a-form-item
          ...
          <tsa-user-select v-model:value="item.value" multiple :placeholder="'请选择' + item.label" modalTitle="候选人员" v-else />
        </a-form-item>
      </template>
      ...
      <a-form-item label="抄送人员" name="copyIds" v-if="eventType !== 'freeApprover' && properties.isCustomCopy">
        <tsa-user-select v-model:value="dataForm.copyIds" multiple allowClear />
      </a-form-item>
      <template v-if="properties.hasOpinion">
        <a-form-item :label="`${label}意见`" name="handleOpinion">
          <tsa-textarea v-model:value="dataForm.handleOpinion" placeholder="请输入" />
          <CommonWordsPopover ref="commonWordsPopoverRef" @confirm="insertOpinion" />
        </a-form-item>
        <a-form-item :label="`${label}附件`" name="fileList">
          <tsa-upload-file v-model:value="dataForm.fileList" type="workFlow" :limit="3" />
        </a-form-item>
      </template>
      <a-form-item
        ...
        <tsa-sign v-model:value="dataForm.signImg" />
      </a-form-item>
    </a-form>
  </BasicModal>
</template>
