...
<script lang="ts" setup>
  import { getInfo, create, update } from '/@/api/businessflow/formDesign';
  ...
  import TableModal from '/@/views/lowcodedev/webDesign/components/TableModal.vue';
  ...
  const [registerForm, { setFieldsValue, getFieldsValue, resetFields, validate, updateSchema }] = useForm({
    schemas: [
      ...
      },
      {
        field: 'interfaceUrl',
        label: '接口地址',
        helpMessage: ['后端接口请求地址', ' 系统将会请求地址中的saveData(post方法), getData(get方法)', '接口例：/bot/demo/UserController'],
        component: 'Input',
        componentProps: { placeholder: '请输入' },
        ifShow: ({ values }) => values.formType == 1,
      },
