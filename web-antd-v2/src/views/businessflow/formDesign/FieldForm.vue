<template>
  <tsa-group-title content="字段设置" :bordered="false" />
  <a-table :data-source="drawingList" :columns="columns" size="small" :pagination="false" rowKey="id" class="search-table">
    <template #bodyCell="{ column, record, index }">
      ...
      <template v-if="column.key === 'tsaDK'">
        <tsa-select v-model:value="record.tsaDK" :options="componentList" :fieldNames="{ options: 'children' }" showSearch allowClear />
      </template>
  ...
  <div class="table-add-action" @click="handleAdd">
    <a-button type="link" preIcon="icon-bts icon-bts-btn-add">新增一行</a-button>
  </div>
</template>
<script lang="ts" setup>
  ...
  const columns = [
    ...
    { title: '控件类型', dataIndex: 'tsaDK', key: 'tsaDK', width: 150 },
  ...
  function getComponentList() {
    const realInputComponents = inputComponents
      .filter(o => !ignoreList.includes(o.__config__.tsaDK))
      .map(o => ({ id: o.__config__.tsaDK, fullName: o.__config__.label }));
    const realSelectComponents = selectComponents
      .filter(o => !ignoreList.includes(o.__config__.tsaDK))
      .map(o => ({ id: o.__config__.tsaDK, fullName: o.__config__.label }));
    const realSystemComponents = systemComponents.map(o => ({ id: o.__config__.tsaDK, fullName: o.__config__.label }));
    componentList.value = [
      {
        id: '1',
        fullName: '基础控件',
  ...
  function handleAdd() {
    let item = { filedName: '', filedId: '', tsaDK: '', required: false, multiple: false };
