<template>
  <BasicModal
    ...
    :closable="false"
    :keyboard="false"
    class="tsa-full-modal full-modal"
    destroyOnClose>
    <template #title>
        ...
        <a-col :span="12" :xxl="10" class="basic-form">
          <BasicForm @register="registerForm">
            <template #sqlTemplate>
              <a-form-item-rest>
                <a-button preIcon="icon-bts icon-bts-btn-add" @click="handleAddSql()">新增SQL</a-button>
                <a-row v-for="(item, i) in sqlTemplate" :key="i" class="mt-10px">
                  ...
                  <a-col :span="2" :offset="1" class="delBtn">
                    <a-button type="danger" preIcon="icon-bts icon-bts-nav-close" @click="handleDelSql(i)"> </a-button>
                  </a-col>
                </a-row>
              </a-form-item-rest>
            </template>
              ...
              <template v-if="column.key === 'relationField'">
                <template v-if="record.sourceType === 2">
                  ...
                  <tsa-date-picker
                    ...
                    placeholder="请选择"
                    clearable />
                  <tsa-input v-else v-model:value="record.relationField" placeholder="请输入" clearable />
                </template>
                <tsa-select v-if="record.sourceType === 4" v-model:value="record.relationField" :options="interfaceSystemOptions" allowClear showSearch />
              </template>
            </template>
            <template #emptyText>
...
<script lang="ts" setup>
  ...
  import { getPrintDevInfo, updatePrintDev, createPrintDev, getFields } from '/@/api/systemmgr/printDev';
