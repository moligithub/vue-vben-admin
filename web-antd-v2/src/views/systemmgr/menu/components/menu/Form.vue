  ...
  <BasicModal v-bind="$attrs" @register="registerModal" :title="getTitle" @ok="handleSubmit" destroy-on-close>
    <BasicForm @register="registerForm">
      <template #icon="{ model, field }">
        <a-row type="flex">
          <div style="flex: 1; margin-right: 10px">
            <tsa-icon-picker v-model:value="model[field]" placeholder="请选择" />
          </div>
          <a-form-item-rest>
            <tsa-color-picker v-model:value="dataForm.propertyJson.iconBackgroundColor" size="small" :predefine="predefineList" name="iconBackground" />
          </a-form-item-rest>
        </a-row>
      </template>
      ...
      <template #moduleId="{ model, field }">
        <TsaTreeSelect v-model:value="model[field]" lastLevel :options="tempData" placeholder="请选择关联" @change="onModuleIdChange" :key="moduleIdKey" />
      </template>
    </BasicForm>
  </BasicModal>
</template>
<script lang="ts" setup>
  ...
  import { getMenuSelector, create, update, getInfo } from '/@/api/systemmgr/menu';
  import { getVisualDevSelector } from '/@/api/lowcodedev/visualDev';
  import { getDictionaryType } from '/@/api/systemdata/dictionary';
  import { getDataReportSelector } from '/@/api/lowcodedev/dataReport';
  import { getDataVSelector } from '/@/api/lowcodedev/dataV';
  import { getPortalSelector } from '/@/api/lowcodedev/portal';
