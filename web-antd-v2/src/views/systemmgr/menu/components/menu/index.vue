<template>
  <BasicPopup v-bind="$attrs" @register="registerPopup" :title="title" class="full-popup">
    <div class="tsa-content-wrapper-search-box ml-10px mt-10px">
      <BasicForm class="search-form" @register="registerSearchForm" @submit="handleSubmit" @reset="handleReset" />
    </div>
    <a-tabs v-model:activeKey="listQuery.category" type="card" class="tsa-content-table-tabs" destroyInactiveTabPane>
      <a-tab-pane key="Web" tab="Web菜单">
        <BasicTable @register="registerWebTable" :columns="menuTableColumns" :searchInfo="getSearchInfo">
          <template #tableTitle>
            ...
            <tsa-upload-btn
              :url="'/bot/systemmgr/Menu/' + systemId + '/Actions/Import'"
...
<script lang="ts" setup>
  ...
  import { getMenuList, delMenu, exportMenu } from '/@/api/systemmgr/menu';
