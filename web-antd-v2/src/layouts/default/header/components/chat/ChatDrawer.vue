    ...
    <a-tabs v-model:activeKey="activeKey" :tabBarGutter="10" class="average-tabs h-full" @change="onTabsChange" size="small">
      <a-tab-pane key="reply" tab="消息">
        <ScrollContainer>
          ...
          <tsa-empty v-if="!replyList.length" />
        </ScrollContainer>
      </a-tab-pane>
      <a-tab-pane key="contacts" tab="联系人">
        ...
        <ScrollContainer ref="infiniteBody" class="user-list">
          ...
          <tsa-empty v-if="!userList.length" />
        </ScrollContainer>
      </a-tab-pane>
    </a-tabs>
    <Im ref="imRef" />
...
<script lang="ts" setup>
  import { getImUser } from '/@/api/permissionmgr/user';
  import { getIMReply, deleteChatRecord, removeChatRecord } from '/@/api/systemmgr/message';
  ...
  import { toDateText } from '/@/utils/tsa';
