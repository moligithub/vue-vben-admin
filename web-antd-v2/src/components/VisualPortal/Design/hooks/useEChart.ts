  ...
  function initChartData() {
    ...
    typeArr.forEach((title, index) => {
      ...
      let obj = {
        name: title,
        type: type,
        stack: styleType == 5 || styleType == 6 || styleType == 7 || (styleType != 4 && tsaDK == 'lineChart') ? title : 'total',
      };
      ...
      series: (() => {
        const list: any[] = (seriesData || []).map((ele: any, index) => {
          let label: any = {
            ...
            position: tsaDK == 'pieChart' ? option.seriesLabelPosition : 'top',
          ...
          if (tsaDK == 'barChart' || tsaDK == 'lineChart' || tsaDK == 'radarChart') itemStyle.color = getColor(index);
          if (tsaDK == 'lineChart') {
    ...
    if (tsaDK == 'barChart' || tsaDK == 'lineChart') {
      if (tsaDK == 'lineChart' && option.areaStyle) xAxis.boundaryGap = false;
