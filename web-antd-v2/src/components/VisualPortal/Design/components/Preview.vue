<template>
  <BasicModal
    ...
    :keyboard="false"
    destroyOnClose
    class="tsa-full-modal full-modal tsa-preview-portal-modal">
    <template #title>
...
<script lang="ts" setup>
  ...
  import { getInfo } from '/@/api/lowcodedev/portal';
  ...
  function filterList(layout) {
    const loop = list => {
      for (let i = 0; i < list.length; i++) {
        const item = list[i];
        if (!(Array.isArray(item.visibility) && item.visibility.includes('pc')) && item.tsaDK) {
          list.splice(i, 1);
          i--;
        }
