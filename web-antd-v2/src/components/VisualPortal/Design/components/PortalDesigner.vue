      ...
      <div class="action-box">
        <div v-for="(item, listIndex) in componentsList" :key="listIndex" class="components-part">
          <a-dropdown placement="bottom">
            <div class="components-title">{{ item.title }}<i class="icon-bts icon-bts-arrow-down"></i></div>
            <template #overlay>
        ...
        <div v-if="showType == 'pc'">
          <grid-layout :layout.sync="layout" :row-height="40" v-if="layout.length">
            <grid-item
              ...
              <div class="mask" v-if="!noNeedMaskList.includes(item.tsaDK)"></div>
                ...
                <draggable v-if="layout.length" class="drawing-board" :list="layout" :animation="340" group="componentsGroup" item-key="i">
                  <template #item="{ element }">
                    <div class="item-box m-10px portal-tabs-box" @click="handleClick(element)" :class="{ 'active-item': element.i === activeId }">
                      ...
                      <div class="mask" v-if="!noNeedMaskList.includes(element.tsaDK)"></div>
...
<script lang="ts" setup>
  ...
  function handleAddComponent(item, type?, currentItem?) {
    ...
    let row = { ...clone, i, x, y };
    if (needDefaultList.includes(row.tsaDK) && type != 'copy') row.option.defaultValue = getDefaultValue(row);
    if (currentItem) {
      if (currentItem.tsaDK == 'card') currentItem.children = [row];
      if (currentItem.tsaDK == 'tab') {
  ...
  function getDefaultValue(row) {
    const tsaDK = row.tsaDK;
    if (tsaDK == 'text') return 'BST快速开发平台';
    // if (tsaDK == 'image') return 'https://app.cdn.bstsoftTTT.com/image/3.2/banner1.png';if (tsaDK == 'image') return 'https://app.cdn.bstsoftTTT.com/image/3.2/banner1.png';
    if (tsaDK == 'image') return './resource/img/banner1.png';
    //if (tsaDK == 'video') return 'https://cdn.bstsoftTTT.com/2022/video/index_video.mp4';
    if (tsaDK == 'video') return './resource/img/index_video.mp4';
    if (tsaDK == 'barChart' || tsaDK == 'lineChart' || tsaDK == 'pieChart') return chartData.baseBarData;
    if (tsaDK == 'radarChart') return chartData.radarData;
    if (tsaDK == 'mapChart') return mapChartData;
    if (tsaDK == 'rankList') return rankList;
    if (tsaDK == 'timeAxis') return timeAxisList;
    if (tsaDK == 'tableList') return tableList;
  }
    ...
    const loop = data => {
      if (data.children && item.children.length) {
        data.children.map(ele => {
          if (ele.tsaDK) emitter.emit('eChart' + ele.i);
    ...
    return new Promise((resolve, reject) => {
      const loop = list => {
        for (let i = 0; i < list.length; i++) {
          ...
          if ((e.tsaDK == 'video' || e.tsaDK == 'image') && option.styleType == 2) {
            const val = e.tsaDK == 'video' ? '视频' : '图片';
