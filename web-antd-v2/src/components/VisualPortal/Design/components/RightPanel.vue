  ...
  <div class="right-box">
    ...
    <div class="filed-box">
      <ScrollContainer class="p-15px !pt-0px">
        <a-form :colon="false" labelAlign="left" :labelCol="{ style: { width: '86px' } }" v-if="activeKey === 1 && activeData && activeData.tsaDK">
          <a-collapse v-model:activeKey="activeName" accordion ghost expandIconPosition="right">
            <Card
              ...
              v-if="activeData.tsaDK != 'tab' && activeData.tsaDK != 'collapse'" />
            <Common
              :key="2"
              :activeData="activeData"
              :showType="showType"
              :menuList="menuList"
              :appMenuList="appMenuList"
              v-if="!noNeedTypeSetList.includes(activeData.tsaDK)" />
            <template v-if="chartList.includes(activeData.tsaDK) || activeData.tsaDK == 'mapChart'">
              <MainTitle :key="3" :activeData="activeData" :showType="showType" />
              <SubTitle :key="4" :activeData="activeData" :showType="showType" />
              <BarStyle :key="5" :activeData="activeData" :showType="showType" v-if="activeData.tsaDK == 'barChart'" />
            </template>
            <XAxis :key="6" :activeData="activeData" :showType="showType" v-if="activeData.tsaDK == 'barChart' || activeData.tsaDK == 'lineChart'" />
            <YAxis :key="7" :activeData="activeData" :showType="showType" v-if="activeData.tsaDK == 'barChart' || activeData.tsaDK == 'lineChart'" />
            <Label :key="8" :activeData="activeData" :showType="showType" v-if="chartList.includes(activeData.tsaDK)" />
            <template v-if="chartList.includes(activeData.tsaDK) || activeData.tsaDK == 'mapChart'">
            ...
            <template v-if="chartList.includes(activeData.tsaDK)">
              <Legend :key="11" :activeData="activeData" :showType="showType" />
              <Color :key="12" :activeData="activeData" :showType="showType" />
            </template>
            <Data :key="13" :activeData="activeData" :showType="showType" v-if="needDataSetList.includes(activeData.tsaDK)" />
            <TableSet :key="14" :activeData="activeData" :showType="showType" v-if="activeData.tsaDK == 'tableList'" />
            <TabSet :key="15" :activeData="activeData" :showType="showType" v-if="activeData.tsaDK == 'tab'" />
            <ScheduleSet :key="16" :activeData="activeData" :showType="showType" v-if="activeData.tsaDK == 'schedule'" />
            <NoticeSet :key="17" :activeData="activeData" :showType="showType" v-if="activeData.tsaDK == 'notice'" />
          </a-collapse>
          <a-form-item label="多端显示" class="!mt-10px">
            <tsa-checkbox v-model:value="activeData.visibility" :options="getVisibilityOptions" />
          </a-form-item>
        </a-form>
...
<script lang="ts" setup>
  ...
  import { getSelectorAll } from '/@/api/systemmgr/menu';
  ...
  const getVisibilityOptions = computed(() => {
    return [
      { id: 'pc', fullName: 'Web' },
      { id: 'app', fullName: 'App', disabled: props.activeData?.tsaDK === 'iframe' },
    ];
  });
