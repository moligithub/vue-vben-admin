<template>
  <div class="h-full" @click.stop="handleClick(item)">
    <component :is="layouts" v-bind="getBindValue" v-if="item.tsaDK !== 'card' && item.tsaDK !== 'tab'" />
    <template v-if="item.tsaDK === 'card'">
...
<script lang="ts" setup>
  ...
  if (props.item.tsaDK !== 'card') layouts.value = markRaw(defineAsyncComponent(() => getComponents(props.item.tsaDK)));
  ...
  const getBindValue = computed(() => ({
    ...
    key: useNeedKeyList.includes(props.item.tsaDK) ? props.item.renderKey : '',
  }));
