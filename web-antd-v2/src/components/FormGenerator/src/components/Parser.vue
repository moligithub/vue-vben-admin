<script lang="tsx">
  ...
  import { getScriptFunc, onlineUtils, getDateTimeUnit, getTimeUnit, getParamList } from '/@/utils/tsa';
    ...
    setup(props, { expose, emit }) {
        ...
        colFormItem(element) {
          ...
          const Item = (
            <render
              ...
              {...listeners}
              ref={config.tsaDK === 'table' ? element.__vModel__ : undefined}
              relations={config.tsaDK === 'table' ? state.relations : undefined}
            />
          );
          ...
          Object.keys(scheme.on).forEach(key => {
            ...
            listeners['on' + upperFirst(key)] = (...arg) => {
              if (key === 'change') {
                ...
                if (['popupSelect', 'relationForm'].includes(config.tsaDK)) setTransferFormData(data, config);
      ...
      function comSet(field, prop, value) {
        ...
        const loop = list => {
          for (let i = 0; i < list.length; i++) {
            ...
            if (item.__vModel__ && item.__vModel__ === prop) {
              switch (field) {
                ...
                case 'endTime':
                  ...
                  break;
                case 'options':
                  if (dyOptionsList.indexOf(item.__config__.tsaDK) > -1) item.options = value;
                  break;
                default:
            ...
            if (item.__config__ && item.__config__.tsaDK !== 'table' && item.__config__.children && Array.isArray(item.__config__.children)) {
      ...
      function initFormData(componentList) {
        ...
        componentList.forEach(cur => {
          ...
          if (cur.__config__.tsaDK == 'table') return;
      ...
      function initRelationForm(componentList) {
        componentList.forEach(cur => {
          const config = cur.__config__;
          if (config.tsaDK == 'relationFormAttr' || config.tsaDK == 'popupAttr') {
            const relationKey = cur.relationField.split('_tsaTable_')[0];
      ...
      function buildRules(componentList) {
        componentList.forEach(cur => {
          ...
          if (config.children && config.tsaDK !== 'table') buildRules(config.children);
        });
      }
      ...
      function buildOptions(componentList) {
        componentList.forEach(cur => {
          ...
          if (config.children && config.tsaDK !== 'table') buildOptions(config.children);
        });
      }
      function buildRelations(componentList, relations) {
        componentList.forEach(cur => {
          ...
          if (config.tsaDK === 'userSelect' && ['dep', 'pos', 'role', 'group'].includes(cur.selectType)) {
