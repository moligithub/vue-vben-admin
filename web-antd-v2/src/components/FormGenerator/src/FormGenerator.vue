          ...
          <a-tooltip title="撤销">
            <div class="undo-btn">
              <a-button type="link" color="info" :disabled="!getCanUndo" @click="handleUndo(replaceDrawingList)">
                <i class="icon-bts icon-bts-report-icon-undo"></i>
              </a-button>
            </div>
          </a-tooltip>
          <a-tooltip title="重做">
            <div class="undo-btn">
              <a-button type="link" color="info" :disabled="!getCanRedo" @click="handleRedo(replaceDrawingList)">
                <i class="icon-bts icon-bts-report-icon-restore"></i>
              </a-button>
            </div>
          </a-tooltip>
        </div>
        <div class="action-bar-right">
          <a-button pre-icon="icon-bts icon-bts-pc" type="link" :class="{ 'unActive-btn': showType != 'pc' }" @click="showType = 'pc'">桌面端</a-button>
          <a-button pre-icon="icon-bts icon-bts-mobile" type="link" :class="{ 'unActive-btn': showType != 'app' }" @click="showType = 'app'">移动端</a-button>
          <a-button pre-icon="icon-bts icon-bts-video-play" type="link" class="unActive-btn" @click="handlePreview">预览</a-button>
          <a-button pre-icon="icon-bts icon-bts-clean" type="link" class="unActive-btn" @click="handleClear">清空</a-button>
        </div>
      </div>
...
<script lang="ts" setup>
    ...
    return new Promise((resolve, reject) => {
      ...
      const loop = list => {
        for (let i = 0; i < list.length; i++) {
          ...
          if (config.layout === 'colFormItem' && !noVModelList.includes(config.tsaDK) && !e.__vModel__) {
          ...
          if (config.tsaDK === 'billRule') {
            if (!config.rule) {
              reject({ msg: '单据组件“单据模板”属性为必填项', target: 1 });
              break;
            }
          }
          if (config.tsaDK === 'relationForm') {
            if (!e.modelId) {
          ...
          if (config.tsaDK === 'popupTableSelect') {
            if (!e.interfaceId) {
          ...
          if (config.tsaDK === 'uploadFile' || config.tsaDK === 'uploadImg') {
          ...
          if (config.tsaDK === 'link' || config.tsaDK === 'iframe') {
  ...
  function createIdAndKey(item, parent: Nullable<GenItem> = null) {
    ...
    if (config.layout === 'colFormItem') {
      if (noVModelList.indexOf(config.tsaDK) < 0 || item.isStorage) {
        item.__vModel__ = !generatorStore.getHasTable ? toggleVmodelCase(`${config.tsaDK}Field${state.idGlobal}`) : '';
      }
      if (parent?.__vModel__ && parent?.__config__.tsaDK === 'table') {
        item.__config__.parentVModel = parent.__vModel__;
      }
    } else if (config.layout === 'rowFormItem') {
      if (config.tsaDK === 'table') {
        item.__vModel__ = toggleVmodelCase(`${config.tsaDK}Field${state.idGlobal}`);
      }
      config.componentName = `${config.tsaDK}${state.idGlobal}`;
  ...
  function shouldClone(_to, _from, target, _event, conf) {
    ...
    if (conf.__config__.tsaDK === 'table') {
      if (noTableAllowList.includes(targetConf.__config__.tsaDK)) {
        return false;
      }
    ...
    if (conf.__config__.tsaDK === 'tableGridTd' && targetConf.__config__.tsaDK === 'tableGrid') return false;
    return true;
  }
  // 预览表单
  ...
  function handleTableAddRow(element, insertPos, cloneRowIdx) {
    ...
    newRow.__config__.tsaDK = 'tableGridTr';
