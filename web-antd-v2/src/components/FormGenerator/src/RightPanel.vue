<template>
  <div class="right-board common-board">
    ...
    <div class="field-box" v-if="activeData?.__config__ && activeData.__config__.tsaDK">
      <ScrollContainer>
        <a-form :colon="false" labelAlign="left" :labelCol="{ style: { width: '90px' } }" v-show="activeKey === 'field'" class="right-board-form">
          <a-form-item label="控件类型" v-if="!['tableGridTr', 'tableGridTd'].includes(activeData.__config__.tsaDK)">
            <a-input v-model:value="getCompName" disabled />
          </a-form-item>
          <a-form-item label="控件用途" v-if="tsaDK == 'calculate'">
            <tsa-select v-model:value="activeData.isStorage" :options="storageTypeOptions" @change="onStorageChange" />
          </a-form-item>
          <div v-if="(activeData.__vModel__ !== undefined && !noVModelList.includes(tsaDK)) || activeData.isStorage">
            <div v-if="getHasTable">
              <div v-if="!config.isSubTable">
                <a-form-item label="数据库表" v-if="tsaDK !== 'table'">
                  <tsa-select
                ...
                <a-form-item label="控件字段">
                  <tsa-select
                    ...
                    showSearch
                    allowClear
                    :disabled="tsaDK === 'table'"
                    :showArrow="tsaDK !== 'table'"
                    ...
                    <template #dropdownRender="{ menuNode: menu }">
                      ...
                      <div class="select-item-add" @mousedown="e => e.preventDefault()" @click="handleAddField">
                        <a-button type="link" preIcon="icon-bts icon-bts-btn-add">添加字段</a-button>
                      </div>
                    </template>
                  </tsa-select>
                </a-form-item>
          ...
          <div v-if="![...layoutList, ...noVModelList].includes(tsaDK) && tsaDK !== 'input'">
            <a-form-item label="是否禁用" v-if="!systemList.includes(tsaDK)">
              <a-switch v-model:checked="activeData.disabled" />
            </a-form-item>
            <a-form-item label="是否必填" v-if="!systemList.includes(tsaDK)">
          ...
          <a-form-item label="是否隐藏" v-if="['relationFormAttr', 'popupAttr', 'calculate'].includes(tsaDK) && activeData.isStorage">
            <a-switch v-model:checked="activeData.__config__.noShow" />
          </a-form-item>
...
<script lang="ts" setup>
  ...
  const tsaDK = computed(() => unref(props.activeData).__config__?.tsaDK);
  ...
  const getCompName = computed(() => {
    ...
    const comp = allComps.filter(o => o.__config__.tsaDK === unref(tsaDK));
    if (!comp.length) return '';
    return comp[0].__config__.label;
  });
  const getRightComp = computed(() => {
    if (!unref(tsaDK)) return null;
    if (orgList.includes(unref(tsaDK))) return RightComp.ROrgRight;
    if (unref(tsaDK) === 'popupTableSelect') return RightComp.RPopupSelect;
    return RightComp['R' + upperFirst(unref(tsaDK))];
  });
  ...
  const drawingOptions = computed(() => {
    ...
    const loop = (data, parent?) => {
      if (!data) return;
      if (data.__config__ && data.__config__.tsaDK !== 'table' && data.__config__.children && Array.isArray(data.__config__.children)) {
        loop(data.__config__.children, data);
      }
      if (Array.isArray(data)) data.forEach(d => loop(d, parent));
      if (data.__vModel__ && data.__config__.tsaDK !== 'table') {
  ...
  watch(
    ...
    val => {
      ...
      if (!val.__config__.tableName && val.__config__.tsaDK !== 'table') {
  ...
  function onChildTableFieldChange(val, formId) {
    ...
    const loop = list => {
      for (let i = 0; i < list.length; i++) {
        ...
        if (config.tsaDK === 'table' && config.tableName === props.activeData.__config__.relationTable) {
        ...
        if (config && config.tsaDK != 'table' && config.children && Array.isArray(config.children)) {
  ...
  function onInputFieldChange(val, formId, parentVModel) {
    ...
    if (parentVModel) {
      const loop = (data, parent?) => {
        ...
        if (Array.isArray(data)) data.forEach(d => loop(d, parent));
        if (parent?.__config__?.tsaDK == 'table') {
          if (
            data.__vModel__ &&
      ...
      const loop = (data, parent?) => {
        if (!data) return;
        if (data.__config__?.tsaDK !== 'table' && data.__config__?.children && Array.isArray(data.__config__.children)) {
          loop(data.__config__.children, data);
        }
        if (Array.isArray(data)) data.forEach(d => loop(d, parent));
  ...
  function onStorageChange(val) {
    if (val == 0) {
      ...
      if (!unref(getHasTable)) {
        props.activeData.__vModel__ = toggleVmodelCase(`${props.activeData.__config__.tsaDK}Field${props.activeData.__config__.formId}`);
      } else {
  ...
  function onChildTableNameChange(tableName, formId) {
    ...
    const loop = list => {
      for (let i = 0; i < list.length; i++) {
        const e = list[i];
        const config = e.__config__;
        if (config.tsaDK === 'table' && config.tableName === tableName && config.formId !== formId) {
          boo = true;
          break;
        }
        if (config && config.tsaDK != 'table' && config.children && Array.isArray(config.children)) {
  ...
  function setDefaultOptions() {
    ...
    if ((props.activeData.__vModel__ === undefined && props.activeData.isStorage != 1) || props.activeData.__config__.tsaDK === 'table') return;
    if (!props.activeData.__config__.tableName || props.activeData.__config__.tableName === unref(getMainTable)) {
      ...
      if (!list.length) {
        ...
        let options = item.fields.map(o => ({
          ...o,
          realField: 'tsa_' + props.activeData.__config__.tableName + '_tsa_' + o.field,
        }));
  ...
  function hasTipLabel(tsaDK) {
    return ![...layoutList, ...systemList].includes(tsaDK) || ['table', 'card', 'billRule'].includes(tsaDK);
  }
  ...
  onMounted(() => {
    ...
    if (!props.activeData.__config__.tableName && props.activeData.__config__.tsaDK !== 'table') {
      props.activeData.__config__.tableName = unref(getMainTable);
    }
    setDefaultOptions();
  });
