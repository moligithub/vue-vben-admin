    ...
    <a-form
      ...
      <template v-if="!['copy', 'remove'].includes(dataForm.value)">
        ...
        <a-form-item label="按钮图标">
          <tsa-icon-picker v-model:value="dataForm.btnIcon" placeholder="请选择" />
        </a-form-item>
      </template>
      ...
      <template v-if="!['copy', 'remove', 'add', 'batchRemove'].includes(dataForm.value)">
        <tsa-group-title content="动作设置" :bordered="false" />
        <a-form-item label="弹窗标题">
          <a-input v-model:value="actionConfig.popupTitle" placeholder="请输入" />
        </a-form-item>
        <a-form-item label="弹窗类型">
          <tsa-radio v-model:value="actionConfig.popupType" :options="popupTypeOptions" optionType="button" button-style="solid" class="right-radio" />
        </a-form-item>
            ...
            <template #bodyCell="{ column, record }">
              ...
              <template v-if="column.key === 'relationField'">
                ...
                <template v-else-if="record.sourceType == 2">
                  <tsa-input-number v-if="['int', 'decimal'].includes(record.dataType)" v-model:value="record.relationField" placeholder="请输入" clearable />
                  <tsa-date-picker
        ...
        <a-form-item label="表单数据" v-else>
          <TsaTreeSelect v-model:value="actionConfig.modelId" :options="treeData" placeholder="请选择" lastLevel allowClear @change="onModeIdChange" />
        </a-form-item>
        ...
        <a-table :data-source="actionConfig.columnOptions" :columns="getColumnOptionsColumns" size="small" :pagination="false">
          ...
          <template #bodyCell="{ column, record, index }">
            ...
            <template v-if="column.key === 'value'">
              ...
              <tsa-select v-else v-model:value="record.value" placeholder="请选择" :options="fieldOptions" showSearch @change="onChange($event, record)" />
            </template>
        ...
        <div class="table-add-action" @click="handleAddColumnOption()">
          <a-button type="link" preIcon="icon-bts icon-bts-btn-add">新增</a-button>
        </div>
        ...
        <a-table :data-source="actionConfig.relationOptions" :columns="relationOptionsColumns" size="small" :pagination="false">
          <template #bodyCell="{ column, record, index }">
            <template v-if="column.key === 'field'">
              <tsa-select v-model:value="record.field" placeholder="请选择" :options="childList" showSearch class="!w-134px" />
            </template>
            <template v-if="column.key === 'type'">
              <tsa-select v-model:value="record.type" placeholder="请选择" :options="typeOptions" @change="onTypeChange(record)" />
            </template>
            <template v-if="column.key === 'value'">
              <template v-if="record.type == 1">
                <a-auto-complete
                  ...
                  @focus="onFocus(record.value)"
                  @search="debounceOnSearch(record.value)" />
                <tsa-select v-else v-model:value="record.value" placeholder="请选择" :options="fieldOptions" showSearch />
              </template>
...
<script lang="ts" setup>
  ...
  import { getVisualDevSelector } from '/@/api/lowcodedev/visualDev';
  import { getFormDataFields } from '/@/api/lowcodedev/visualDev';
  ...
  const formFieldsOptions = computed(() => {
    ...
    const loop = (data, parent?) => {
      ...
      if (data.__vModel__ && !noAllowList.includes(data.__config__.tsaDK))
        list.push({
          id: data.__vModel__,
  ...
  function init(data) {
    ...
    childList.value = cloneDeep(data.children)
      .filter(o => o.__vModel__ && !noAllowSelectList.includes(o.__config__.tsaDK))
      .map(o => ({
        id: o.__vModel__,
  ...
  function onRelationFieldChange(val, row) {
    if (!val) return (row.tsaDK = '');
    let list = unref(formFieldsOptions).filter(o => o.__vModel__ === val);
    if (!list.length) return (row.tsaDK = '');
    let item = list[0];
    row.tsaDK = item.__config__.tsaDK;
  }
