<template>
  <a-form-item label="合计设置">
    <tsa-switch v-model:value="activeData.showSummary" />
  </a-form-item>
  <a-form-item label="合计字段" v-if="activeData.showSummary">
    <tsa-select v-model:value="activeData.summaryField" placeholder="请选择" :options="childrenList" allowClear showSearch multiple />
  </a-form-item>
    ...
    <template #item="{ element }">
      <div class="custom-draggable-item">
        ...
        <div class="edit-btn custom-line-icon" @click="editBtnConfig(element)">
          <i class="icon-bts icon-bts-btn-edit" />
        </div>
      </div>
    </template>
  </draggable>
  ...
  <div class="custom-draggable-list">
    ...
    <div class="add-btn" v-if="activeData.footerBtnsList?.length < 5">
      <a-button type="link" preIcon="icon-bts icon-bts-btn-add" @click="addBtn()">添加按钮</a-button>
    </div>
  </div>
...
<script lang="ts" setup>
  ...
  const defaultColumnBtnsList = [
    { value: 'copy', label: '复制', show: true, btnType: 'primary', btnIcon: 'icon-bts icon-bts-btn-edit' },
    { value: 'remove', label: '删除', show: true, btnType: 'danger', btnIcon: 'icon-bts icon-bts-btn-clearn', showConfirm: true },
  ];
  const defaultFooterBtnsList = [
    { value: 'add', label: '添加', show: true, btnType: 'primary', btnIcon: 'icon-bts icon-bts-btn-add' },
    { value: 'batchRemove', label: '批量删除', show: true, btnType: 'danger', btnIcon: 'icon-bts icon-bts-btn-clearn', showConfirm: true },
  ];

  const childrenList = computed(() => {
    const list = props.activeData.__config__.children.filter(
      o => ['input', 'inputNumber', 'calculate'].includes(o.__config__.tsaDK) && o.__vModel__ && !o.useMask,
    );
    return list.map(o => ({ id: o.__vModel__, fullName: o.__config__.label }));
  });
  const getInputComponents = computed(() => inputComponents.filter(o => !noTableAllowList.includes(o.__config__.tsaDK)));
  const getSelectComponents = computed(() => {
    const list = selectComponents.filter(o => !noTableAllowList.includes(o.__config__.tsaDK));
    return props.formInfo.type == 1 || props.formInfo.type == 6 ? [...list, calculateItem] : list;
  });
  const getSystemComponents = computed(() => systemComponents.filter(o => !noTableAllowList.includes(o.__config__.tsaDK)));
