...
<script lang="ts" setup>
  ...
  import { getConfigData } from '/@/api/lowcodedev/visualDev';
  ...
  import { getWebhookUrl } from '/@/api/lowcodedev/integrate';
  ...
  function transformFieldList(formFieldList) {
    ...
    const loop = (data, parent?) => {
      ...
      if (data.__vModel__ && data.__config__.tsaDK !== 'table') {
        ...
        if (dyOptionsList.includes(config.tsaDK) && config.dataType !== 'static') delete item.options;
        list.push(item);
      }
    ...
    outer: for (let i = 0; i < list.length; i++) {
      ...
      for (let j = 0; j < e.groups.length; j++) {
        ...
        if (
          ...
          (!['inputNumber', 'calculate'].includes(child.tsaDK) || (['inputNumber', 'calculate'].includes(child.tsaDK) && child.fieldValue !== 0))
        ) {
