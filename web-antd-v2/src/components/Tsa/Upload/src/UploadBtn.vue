<template>
  <div class="common-container">
    <a-button :type="buttonType" :pre-icon="showIcon ? 'icon-bts icon-bts-btn-upload' : ''" @click="openModal">
      ...
      <div class="px-10 py-20px">
        <a-form :colon="false" labelAlign="right" :labelCol="{ style: { width: '90px' } }" :model="dataForm" :rules="rules" ref="formElRef">
          <a-form-item label="上级菜单" name="parentId" v-if="type == 'menu'">
            <tsa-tree-select v-model:value="dataForm.parentId" :options="treeOptions" placeholder="请选择" :lastLevel="props.data.category == 'App'" />
          </a-form-item>
          ...
          <a-form-item label="上传文件" name="fileList">
            <a-upload :file-list="dataForm.fileList" :accept="accept" :before-upload="beforeUpload" @remove="handleRemove">
              <a-button preIcon="icon-bts icon-bts-btn-upload" @click="uploadFile">上传文件</a-button>
...
<script lang="ts" setup>
  ...
  import { getMenuSelector } from '/@/api/systemmgr/menu';
  ...
  defineOptions({ name: 'TsaUploadBtn', inheritAttrs: false });
  ...
  const hideList = ['/systemmgr/DataModel', '/systemmgr/DataInterfaceVariate'];
