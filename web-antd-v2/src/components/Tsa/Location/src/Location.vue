<template>
  <div :class="`${prefixCls}`">
    <a-button preIcon="icon-bts icon-bts-daka" @click="handleLocation()" v-if="!detailed">{{ innerValue.fullAddress ? '重新定位' : '添加定位' }}</a-button>
  ...
  <a-modal
    ...
    <div class="location-contain">
      ...
      <ScrollContainer ref="infiniteBody" class="around-contain" v-loading="loading && listQuery.currentPage == 1">
        ...
        <!-- 请选择允许微调范围内的地点 -->
        <tsa-empty class="!mt-100px" v-if="!list.length && !loading" />
      </ScrollContainer>
    </div>
  </a-modal>
</template>
...
<script lang="ts" setup>
  ...
  defineOptions({ name: 'TsaLocation', inheritAttrs: false });
  const defaultValue = {
    lat: '',
    lng: '',
    name: '',
  ...
  const staticMapUrl = computed(() => {
    ...
    const url = `${globSetting.apiUrl}/bot/systemmgr/Location/staticmap?location=${location}&zoom=19&size=80*80&key=${globSetting.aMapWebKey}`;
    return url;
  });
