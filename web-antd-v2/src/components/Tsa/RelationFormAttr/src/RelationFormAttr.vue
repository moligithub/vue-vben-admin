...
<script lang="ts" setup>
  ...
  defineOptions({ name: 'TsaRelationFormAttr', inheritAttrs: false });
  ...
  onBeforeMount(() => {
    emitter.on('setRelationData', data => {
      if (!props.isStorage || !props.showField || !props.relationField || props.relationField != data.tsaRelationField) return;
