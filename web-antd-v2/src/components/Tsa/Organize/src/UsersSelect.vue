<template>
  <div :class="[$attrs.class, 'select-tag-list']" v-if="buttonType === 'button'">
    <a-button preIcon="icon-bts icon-bts-btn-add" @click="openSelectModal">{{ modalTitle }}</a-button>
    <div class="tags">
      ...
      <div class="transfer-pane left-pane">
        ...
        <div class="transfer-pane__body transfer-pane__body-tab" v-if="selectType === 'all'">
          ...
          <template v-if="activeKey === '1'">
            ...
            <ScrollContainer v-loading="loading && pagination.currentPage === 1" v-else ref="infiniteBody">
              ...
              <tsa-empty v-if="!treeData.length" />
            </ScrollContainer>
          </template>
          <BasicTree
            class="tree-main"
        ...
        <div class="transfer-pane__body transfer-pane__body-tab" v-else>
          ...
          <ScrollContainer v-loading="loading && pagination.currentPage === 1" ref="infiniteBody">
            ...
            <tsa-empty v-if="!ableList.length" />
          </ScrollContainer>
        </div>
      </div>
      <div class="transfer-pane right-pane">
        ...
        <div class="transfer-pane__body">
          <ScrollContainer>
            ...
            <tsa-empty v-if="!selectedData.length" />
          </ScrollContainer>
        </div>
      </div>
    </div>
...
<script lang="ts" setup>
  import { getImUserSelector, getSelectedList, getSelectedUserList } from '/@/api/permissionmgr/user';
  ...
  const defaultSelectedList = [
    { id: 'department', type: 'department', fullName: '部门', icon: 'icon-bts icon-bts-tree-department1', children: [] },
    { id: 'position', type: 'position', fullName: '岗位', icon: 'icon-bts icon-bts-tree-position1', children: [] },
    { id: 'user', type: 'user', fullName: '用户', icon: 'icon-bts icon-bts-tree-user2', children: [] },
    { id: 'group', type: 'group', fullName: '分组', icon: 'icon-bts icon-bts-generator-group1', children: [] },
    { id: 'role', type: 'role', fullName: '角色', icon: 'icon-bts icon-bts-generator-role', children: [] },
    { id: 'system', type: 'system', fullName: '动态参数', icon: 'icon-bts icon-bts-tree-user2', children: [] },
  ];
  defineOptions({ name: 'TsaUsersSelect', inheritAttrs: false });
  ...
  const systemFieldList = ref<any[]>([
    { id: '@currentOrg', fullName: '当前组织', type: 'system', headIcon: '/bot/filehandler/Image/userAvatar/001.png' },
    { id: '@currentOrgAndSubOrg', fullName: '当前组织及子组织', type: 'system', headIcon: '/bot/filehandler/Image/userAvatar/001.png' },
    { id: '@currentGradeOrg', fullName: '当前分管组织', type: 'system', headIcon: '/bot/filehandler/Image/userAvatar/001.png' },
  ]);
