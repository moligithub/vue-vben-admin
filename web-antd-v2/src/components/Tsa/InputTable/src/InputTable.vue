        ...
        <template v-for="(item, cIndex) in rowConfig">
          <template v-if="column.key === item.__vModel__ && column.__config__.formId === item.__config__.formId">
            <div :key="item.__config__.formId">
              <TsaRelationForm
                v-if="item.__config__.tag === 'TsaRelationForm'"
                :rowIndex="index"
...
<script lang="ts" setup>
  ...
  import { getScriptFunc, getDateTimeUnit, thousandsFormat } from '/@/utils/tsa';
  ...
  defineOptions({ name: 'TsaInputTable', inheritAttrs: false });
  ...
  function buildOptions() {
    state.rowConfig.forEach((cur, index) => {
      ...
      if (dyOptionsList.indexOf(config.tsaDK) > -1) {
    ...
    for (let key in currRelations) {
      if (key === data.__vModel__) {
        for (let i = 0; i < currRelations[key].length; i++) {
          ...
          const tsaDK = config.tsaDK;
          let defaultValue: any = null;
          if (
            ['checkbox', 'cascader', 'organizeSelect'].includes(tsaDK) ||
            (['select', 'treeSelect', 'popupSelect', 'popupTableSelect', 'userSelect'].includes(tsaDK) && e.multiple)
          ) {
            defaultValue = [];
          }
  ...
  function buildRowAttr(rowIndex) {
    ...
    for (let i = 0; i < row.length; i++) {
      ...
      if (config.tsaDK === 'userSelect' && cur.relationField && cur.selectType !== 'all' && cur.selectType !== 'custom') {
        let value = getFieldVal(cur.relationField, rowIndex) || [];
        cur.ableRelationIds = Array.isArray(value) ? value : [value];
      }
      if (config.tsaDK === 'datePicker' || config.tsaDK === 'timePicker') {
  ...
  function onFormDataChange(rowIndex, colIndex, val, row) {
    ...
    if (['popupSelect', 'relationForm'].includes(data.tsaDK)) setTransferFormData(row, data.config.__config__);
  }
  ...
  function getConfById(formId, rowIndex) {
    ...
    let newObj = {};
    item = getRealProps(item, itemConfig.tsaDK);
    ...
    if (['relationForm', 'popupSelect'].includes(itemConfig.tsaDK)) {
      newObj['field'] = props.config.__vModel__ + item.__vModel__ + '_tsaRelation_' + rowIndex;
    }
    if (['relationFormAttr', 'popupAttr'].includes(itemConfig.tsaDK)) {
      let prop = newObj['relationField'].split('_tsaTable_')[0];
      newObj['relationField'] = props.config.__vModel__ + prop + '_tsaRelation_' + rowIndex;
    }
    return newObj;
  }
  /**
  ...
  function getEmptyRow(val) {
    return state.rowConfig.map((t: any) => ({
      ...
      options: dyOptionsList.indexOf(t.__config__.tsaDK) > -1 ? t.options : [],
      valid: true,
      regValid: true,
      regErrorText: '',
      on: t.on || {},
      tsaDK: t.__config__.tsaDK,
  ...
  function getDefaultEmptyValue(item) {
    if (!item.__config__ || !item.__config__.tsaDK) return undefined;
    const tsaDK = item.__config__.tsaDK;
    const list = ['checkbox', 'uploadFile', 'uploadImg', 'cascader', 'organizeSelect', 'areaSelect'];
    return list.includes(tsaDK) || item.multiple ? [] : undefined;
  }
  // 获取表格数据
  ...
  function batchRemoveRow(item) {
    ...
    const handleBatchRemove = () => {
      state.tableFormData = state.tableFormData.filter(o => !state.selectedRowKeys.includes(o.tsaId));
      state.dataSource = state.dataSource.filter(o => !state.selectedRowKeys.includes(o.tsaId));
      nextTick(() => {
