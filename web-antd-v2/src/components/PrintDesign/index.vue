  ...
  <div class="print-template-detail">
    <div class="print-template-detail__body">
      <div class="system-view-nav">
        <ScrollContainer>
          <div class="tsa-common-title"><h2>表单字段</h2></div>
          <BasicLeftTree
            ref="leftTreeRef"
            :showSearch="false"
...
<script lang="ts" setup>
  ...
  const state = reactive<State>({
    ...
    funData: [
      ...
      { fullName: '数据字典(字段,字典分类)', id: 'tsaDictionary' },
      { fullName: '日期选择(字段,日期格式)', id: 'tsaDate' },
    ],
  });
  ...
  function handleNodeClick(_item, node) {
    ...
    const tableParent = getCurrentParentByTag('table[data-tsa-table-tag="table"]');
    if (!tableParent) {
      if (item == 'tsaDictionary') return openDictionaryModal(true, { node });
  ...
  function reload(item, node) {
    unref(editor).insertContent(getSpanNode('tsaDictionary', node, item));
    state.content = unref(editor).getContent({ format: 'html' });
  }
  function getEditConfig(e) {
    ...
    state.initConfig = {
      ...
      content_style: `html {
        ...
        .tsa-tiny-color--customer {
          color: #005AF3;
        }
        .tsa-tiny-color--contacts {
          color: #1CB24C;
        }
        .tsa-tiny-color--business {
          color: #FF852F;
        }
        .tsa-tiny-color--contract {
          color: #FF4A43;
        }
        .tsa-tiny-color--receivables {
          color: #FFAE46;
        }
        .tsa-tiny-color--product {
          color: #00D0D4;
        }
        .tsa-tiny-color--common {
          color: #2362FB;
        }
      ...
      setup: editor => {
        editor.ui.registry.addButton('page', {
          text: `<i class="icon-bts icon-bts-orderExample" style="font-size:18px"></i>`,
          tooltip: '纸张大小',
          onAction: function () {
        ...
        editor.on('keydown', e => {
          ...
          if (e.keyCode === 8 || e.keyCode == 46) {
            ...
            if (currentNode.getAttribute('data-tsa-table-td-tag') === 'value') {
              e.preventDefault();
              return false;
  ...
  function getSpanNode(id, node, item?) {
    ...
    if (id == 'tsaDictionary') return `<p style="display:inline-block" data-tag='tsaDictionary'>数据字典(${parent},"${item}")<p>`;
    if (id == 'tsaDate') return `<p style="display:inline-block" data-tag='tsaDate'>日期选择(${parent},"yyyy-MM-dd")<p>`;
    return `<span data-tag="${parent}.${id}" class="tsa-print-tag-span ${getSpanColorClass()}" contenteditable="false">{${id}}</span>`;
  }
