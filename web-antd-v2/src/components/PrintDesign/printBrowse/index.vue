<template>
  <BasicModal v-bind="$attrs" @register="registerModal" defaultFullscreen :footer="null" :closable="false" :keyboard="false" class="tsa-full-modal full-modal">
    <template #title>
      <div class="tsa-full-modal-header">
        <div class="header-title">
          <img src="../../../assets/images/tsa.png" class="header-logo" />
...
<script lang="ts" setup>
  ...
  import { getData, getBatchData, createPrintLog } from '/@/api/systemmgr/printDev';
  import { useGlobSetting } from '/@/hooks/setting';
  import { buildBitUUID } from '/@/utils/uuid';
  import JsBarcode from 'jsbarcode';
  import { toDataURL } from 'qrcode';
  import { useUserStore } from '/@/store/modules/user';
  import { getAmountChinese } from '/@/utils/tsa';
  import dayjs from 'dayjs';
  import { useBaseStore } from '/@/store/modules/base';
  import { getDateFormat } from '/@/utils/tsa';
  ...
  function handleReplaceValue(dom, tag) {
    ...
    for (let i = 0; i < dataList.length; i++) {
      ...
      if (elementText.includes('{') && elementText.includes('data-tag')) {
        ...
        if (tag == 'td') {
          let value = getValueFun(elementText);
          let spanText = elementText.match(/<span class="tsa-print-tag-span.*?[^}]}.*?<\/span>/);
  ...
  function replaceCommonValue() {
    const list: any[] = state.printTemplate.match(/<span class="tsa-print-tag-span.*?[^}]}.*?<\/span>/g) || [];
    for (let i = 0; i < list.length; i++) {
      if (list[i].includes('{') && list[i].includes('data-tag')) {
        let value = getValue(list[i]);
        let spanText = list[i].match(/<span class="tsa-print-tag-span.*?[^}]}.*?<\/span>/);
        if (spanText?.length) {
          spanText = spanText[0];
          let fieldStr = spanText.match(/{.*}/g);
  ...
  function getTableKey(dom) {
    ...
    for (let tdDom of tdDoms) {
      ...
      if (['thousands', 'isAmountChinese', 'tsaDictionary', 'tsaDate'].includes(dataTag)) {
  ...
  function replaceSysValue() {
    ...
    if (recordList.length) {
      ...
      for (let i = 0; i < recordList.length; i++) {
        ...
        content += `<tr><td style="width: 20%;" data-mce-style="width: 20%;"><span class="tsa-print-tag-span tsa-tiny-color--common" contenteditable="false">${
          record.nodeName
        }</span></td><td style="width: 20%;" data-mce-style="width: 20%;"><span class="tsa-print-tag-span tsa-tiny-color--common" contenteditable="false">${
          record.userName
        }</span></td><td style="width: 20%;" data-mce-style="width: 20%;"><span class="tsa-print-tag-span tsa-tiny-color--common" contenteditable="false">${dayjs(
          record.handleTime,
        ).format(
          'YYYY-MM-DD HH:mm:ss',
        )}</span></td><td style="width: 20%;" data-mce-style="width: 20%;"><span class="tsa-print-tag-span tsa-tiny-color--common" contenteditable="false">${getHandleName(
          record.handleStatus,
        )}</span></td><td style="width: 20%;" data-mce-style="width: 20%;"><span class="tsa-print-tag-span tsa-tiny-color--common" contenteditable="false">${
          record.handleOpinion || ''
        }</span></td><td style="width: 100px" data-mce-style="width: 20%;"><span class="tsa-print-tag-span tsa-tiny-color--common" contenteditable="false">${
