  ...
  <div class="common-container">
    <template v-if="config.popupType === 'dialog'">
      <a-modal
        ...
        <div class="tsa-common-search-box tsa-common-search-box-modal">
          ...
          <div class="tsa-common-search-box-right">
            <a-tooltip placement="top">
              ...
              <RedoOutlined class="tsa-common-search-box-right-icon" @click="initData" />
            </a-tooltip>
          </div>
        </div>
...
<script lang="ts" setup>
  ...
  import { getFieldDataSelect } from '/@/api/lowcodedev/visualDev';
  ...
  function handleSubmit() {
    ...
    for (let i = 0; i < unref(selectRow).length; i++) {
      ...
      for (let j = 0; j < props.config.relationOptions.length; j++) {
        ...
        item[row.field] = row.type === 1 ? e[!unref(getIsDynamic) ? row.value + '_tsaId' : row.value] : row.value;
      }
      checked.push(item);
    }
    emit('select', checked);
  ...
  function getParamList() {
    ...
    for (let i = 0; i < templateJson.length; i++) {
      ...
      if (e.tsaDK === 'createUser') e.defaultValue = unref(getUserInfo).userId;
      if (e.tsaDK === 'createTime') e.defaultValue = new Date().getTime();
      if (e.tsaDK === 'currOrganize') e.defaultValue = unref(getUserInfo).organizeId;
      if (e.tsaDK === 'currPosition' && unref(getUserInfo).positionIds?.length) e.defaultValue = unref(getUserInfo).positionIds[0].id;
    }
    return templateJson;
  }
