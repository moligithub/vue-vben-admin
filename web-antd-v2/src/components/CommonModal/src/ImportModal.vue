  ...
  <BasicModal
    ...
    <div class="import-main" v-show="activeStep == 2">
      ...
      <div class="unsuccess" v-if="result.resultType">
        ...
        <div class="conTips">
          <p>以下文件数据为导入异常数据</p>
          <a-button type="link" preIcon="icon-bts icon-bts-btn-download" @click="handleExportExceptionData()">导出异常数据</a-button>
        </div>
...
<script lang="ts" setup>
  ...
  function init(data) {
    state.actionUrl = `/bot/${data.url || 'lowcodedev/OnlineDev'}/Uploader`;
    state.apiUrl = data.url || `lowcodedev/OnlineDev/${data.modelId}`;
    ...
    if (state.activeStep == 0) {
      ...
      getImportPreview(state.apiUrl, { fileName: state.fileName })
        .then(res => {
          ...
          state.resultColumnsChildList = state.resultColumns.filter(o => o.tsaDK === 'table' && o.children && o.children.length);
          const preHeaderList = cloneDeep(headerList);
          for (let i = 0; i < preHeaderList.length; i++) {
            if (preHeaderList[i].children?.length && preHeaderList[i]?.tsaDK == 'table') {
          ...
          state.columnsChildList = state.columns.filter(o => o.tsaDK === 'table' && o.children && o.children.length);
          state.btnLoading = false;
          state.activeStep += 1;
        })
        .catch(() => {
  ...
  function getMergeList(list) {
    list.forEach(item => {
      if (item.children && item.children.length && item?.tsaDK == 'table') {
