<template>
  <BasicModal
    ...
    @ok="handleSubmit"
    destroyOnClose
    class="tsa-super-query-modal tsa-condition-modal">
          ...
          <template #content>
            <div class="plan-list" v-if="planList.length">
              <div class="plan-list-item" v-for="(item, i) in planList" :key="i" @click="selectPlan(item)">
                ...
                <i class="icon-bts icon-bts-nav-close" @click.stop="delPlan(item.id, i)"></i>
              </div>
            </div>
...
<script lang="ts" setup>
  ...
  import { getAdvancedQueryList, delAdvancedQuery, create, update } from '/@/api/systemmgr/advancedQuery';
  ...
  const state = reactive<State>({
    planList: [],
    conditionList: [
      { logic: 'and', groups: [{ field: '', symbol: '', tsaDK: '', fieldValueType: 2, fieldValue: undefined, fieldValueTsaDK: '', cellKey: +new Date() }] },
    ],
