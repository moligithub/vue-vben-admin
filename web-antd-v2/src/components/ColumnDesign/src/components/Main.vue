<template>
  <div class="column-design-container">
    <div class="main-board">
      <tsa-group-title content="查询字段" :bordered="false" />
      <a-table :data-source="columnData.searchList" :columns="searchColumns" size="small" :pagination="false" rowKey="id" class="search-table">
        <template #bodyCell="{ column, record, index }">
          <template v-if="column.key === 'drag'">
            <i class="drag-handler icon-bts icon-bts-darg" title="点击拖动" />
          </template>
          <template v-if="column.key === 'label'">
            <a-input v-model:value="record.label" placeholder="请输入" allowClear />
          </template>
          <template v-if="column.key === 'tsaDK'">
            <tsa-select
              v-model:value="record.tsaDK"
              :options="viewTsaDKOptions"
              :disabled="record.__config__.isFromParam"
              class="!w-160px"
              @change="onTsaDKChange($event, record, index)" />
            <template v-if="canSetAttrs.includes(record.tsaDK) && !record.__config__.isFromParam">
              <i class="icon-bts icon-bts-shezhi cursor-pointer ml-8px leading-30px" title="组件属性设置" @click="openExtraConfig(record, index)" />
            </template>
          </template>
          <template v-if="column.key === 'searchType'">
            <tsa-select
              ...
              :disabled="!['input', 'textarea'].includes(record.tsaDK)"
              v-if="!record.__config__.isFromParam" />
            <span v-else></span>
          </template>
          <template v-if="column.key === 'value'">
            <template v-if="showInputList.includes(record.tsaDK)">
              <a-input v-model:value="record.value" placeholder="请输入" allowClear />
            </template>
            <template v-if="record.tsaDK === 'inputNumber'">
              <tsa-number-range v-model:value="record.value" :precision="record.precision" :disabled="record.disabled" />
            </template>
            <template v-if="['rate', 'slider'].includes(record.tsaDK)">
              <tsa-number-range v-model:value="record.value" :precision="record.allowHalf ? 1 : 0" :disabled="record.disabled" />
            </template>
            <template v-if="showSelectList.includes(record.tsaDK)">
              <tsa-select
            ...
            <template v-if="record.tsaDK === 'datePicker'">
              <tsa-date-range v-model:value="record.value" :format="record.format" allowClear />
            </template>
            <template v-if="record.tsaDK === 'timePicker'">
              <tsa-time-range v-model:value="record.value" :format="record.format" allowClear />
            </template>
            <template v-if="record.tsaDK === 'roleSelect'">
              <tsa-role-select
                v-model:value="record.value"
            ...
            <template v-if="record.tsaDK === 'groupSelect'">
              <tsa-group-select
                v-model:value="record.value"
                :selectType="record.selectType"
                :ableIds="record.ableIds"
            ...
            <template v-if="record.tsaDK === 'areaSelect'">
              <tsa-area-select v-model:value="record.value" :level="record.level" :multiple="record.searchMultiple" :key="record.cellKey" allowClear />
            </template>
            <template v-if="record.tsaDK === 'organizeSelect'">
              <tsa-organize-select
                v-model:value="record.value"
                :selectType="record.selectType"
                :ableIds="record.ableIds"
            ...
            <template v-if="record.tsaDK === 'cascader'">
              <tsa-cascader
                v-model:value="record.value"
                placeholder="请选择"
                :options="record.options"
            ...
            <template v-if="record.tsaDK === 'depSelect'">
              <tsa-dep-select
                v-model:value="record.value"
                :selectType="record.selectType"
                :ableIds="record.ableIds"
            ...
            <template v-if="record.tsaDK === 'posSelect'">
              <tsa-pos-select
                v-model:value="record.value"
                :selectType="record.selectType"
                :ableIds="record.ableIds"
            ...
            <template v-if="record.tsaDK === 'userSelect'">
              <tsa-user-select
                v-model:value="record.value"
            ...
            <template v-if="record.tsaDK === 'usersSelect'">
              <tsa-users-select
                v-model:value="record.value"
                :selectType="record.selectType"
                :ableIds="record.ableIds"
            ...
            <template v-if="record.tsaDK === 'autoComplete'">
              <tsa-auto-complete
                v-model:value="record.value"
          ...
          <template v-if="column.key === 'isKeyword'">
            <a-checkbox v-model:checked="record.isKeyword" :disabled="!canSetKeyword.includes(record.tsaDK) || (getIsKeywordDisabled && !record.isKeyword)" />
          </template>
          <template v-if="column.key === 'searchMultiple'">
            <a-checkbox
              v-model:checked="record.searchMultiple"
              :disabled="!multipleList.includes(record.tsaDK)"
              @change="onSearchMultipleChange(record, index)" />
          </template>
        </template>
      </a-table>
      <tsa-group-title content="列表字段" :bordered="false" class="mt-20px" />
        ...
        <template #bodyCell="{ column, record }">
          ...
          <template v-if="column.key === 'sortable'">
            <a-checkbox
              ...
              :disabled="
                record.id.indexOf('_tsa_') > 0 || (record.__config__ && record.__config__.isSubTable) || noGroupList.includes(record.__config__.tsaDK)
              " />
          </template>
          ...
          <template v-if="column.key === 'align'">
            <tsa-select v-model:value="record.align" :options="alignOptions" />
          </template>
          ...
          <a-form :colon="false" labelAlign="left" :labelCol="{ style: { width: '90px' } }" class="right-board-form">
            ...
            <div v-if="columnData.type == 2">
              ...
              <div v-if="columnData.treeDataSource === 'dictionary'">
                <a-form-item label="数据字典">
                  <tsa-tree-select v-model:value="columnData.treeDictionary" :options="dicOptions" lastLevel allowClear placeholder="请选择" />
                </a-form-item>
              ...
              <div v-if="columnData.treeDataSource === 'api'">
                <a-form-item label="数据加载">
                  <tsa-radio v-model:value="columnData.treeSyncType" :options="treeSyncTypeOptions" />
                </a-form-item>
            ...
            <div v-if="columnData.type !== 3 && columnData.type !== 5">
              ...
              <a-form-item label="分页条数" v-if="columnData.hasPage">
                <tsa-radio v-model:value="columnData.pageSize" :options="pageSizeOptions" optionType="button" button-style="solid" class="right-radio" />
              </a-form-item>
            ...
            <a-form-item label="子表样式" v-if="(columnData.type == 1 || columnData.type == 2) && webType != 4">
              <tsa-select v-model:value="columnData.childTableStyle" :options="childTableStyleOptions" placeholder="请选择" />
            </a-form-item>
            ...
            <div v-if="modelType == 1">
              ...
              <div class="custom-btns-list">
                ...
                <div class="add-btn">
                  <a-button type="link" preIcon="icon-bts icon-bts-btn-add" @click="addCustomBtn">添加按钮</a-button>
                </div>
              </div>
            </div>
            <div v-if="webType != 4">
...
<script lang="ts" setup>
  ...
  import { getPrintDevSelector } from '/@/api/systemmgr/printDev';
  ...
  const defaultBtnsOption = [
    { value: 'add', icon: 'icon-bts icon-bts-btn-add', label: '新增' },
    { value: 'download', icon: 'icon-bts icon-bts-btn-download', label: '导出' },
    { value: 'upload', icon: 'icon-bts icon-bts-btn-upload', label: '导入' },
    { value: 'batchRemove', icon: 'icon-bts icon-bts-btn-clearn', label: '批量删除' },
    { value: 'batchPrint', icon: 'icon-bts icon-bts-report-icon-preview-printPreview', label: '批量打印' },
  ];
  const btnsOption = ref(defaultBtnsOption);
  const columnBtnsOption = ref([
    { value: 'edit', icon: 'icon-bts icon-bts-btn-edit', label: '编辑' },
    { value: 'remove', icon: 'icon-bts icon-bts-btn-clearn', label: '删除' },
    { value: 'detail', icon: 'icon-bts icon-bts-generator-menu', label: '详情' },
  ]);
  ...
  const searchColumns = computed(() => {
    let list = [
      ...
      { title: '列名', dataIndex: 'label', key: 'label', width: 200 },
      { title: '字段', dataIndex: 'prop', key: 'prop' },
      { title: '输入类型', dataIndex: 'tsaDK', key: 'tsaDK', width: 200 },
    ...
    if (unref(webType) == 4) {
      ...
      list = list.filter(o => o.dataIndex != 'tsaDK');
    }
    return list;
  });
  ...
  const formFieldsOptions = computed(() => {
    ...
    const loop = (data, parent?) => {
      ...
      if (data.__config__ && data.__config__.tsaDK) {
        ...
        if (data.__config__.layout === 'colFormItem' && data.__vModel__ && visibility) {
          const isTableChild = parent && parent.__config__ && parent.__config__.tsaDK === 'table';
          list.push({
  ...
  const viewFieldOptions = computed(() => {
    ...
    return props.viewFields.map(o => ({ id: o.defaultValue, fullName: o.field, __vModel__: o.defaultValue, __config__: { tsaDK: 'input' } }));
  });
  const selectOptions = computed(() => {
    let options = unref(formFieldsOptions).map(o => ({ ...o, disabled: false }));
    if (state.columnData.treeDataSource === 'formField') options = options.filter(o => ['organizeSelect', 'depSelect'].includes(o.__config__.tsaDK));
    return options;
  });
  const summaryFieldOptions = computed(() =>
    state.groupFieldOptions.filter(o => ['input', 'inputNumber', 'calculate'].includes(o.__config__.tsaDK) && o.__vModel__ && !o.useMask),
  );
  ...
  function editDefaultSortConfig() {
    openDefaultSortConfigModal(true, {
      ...
      columnOptions: state.groupFieldOptions.filter(o => o.id.indexOf('_tsa_') < 0),
    });
  }
  ...
  function onTsaDKChange(val, record, i) {
    record.__config__.tsaDK = val;
    let defaultItem: any = {
      id: record.id,
    ...
    outer: for (let i = 0; i < data.length; i++) {
      inter: for (let ii = 0; ii < defaultData.length; ii++) {
        if (data[i].prop === defaultData[ii].prop) {
          ...
          if (type === 'search') {
            if (data[i].tsaDK === defaultData[ii].tsaDK) {
  ...
  function setDefaultUpLoadData() {
    ...
    for (let i = 0; i < unref(formFieldsOptions).length; i++) {
      ...
      const tsaDK = e.__config__.tsaDK;
      if (!noUploadList.includes(tsaDK) && (required || systemComponentsList.includes(tsaDK)) && !newList.includes(e.id)) {
        newList.push(e.id);
      }
    }
  ...
  function init(list) {
    const columnOptions = list.filter(o => !noColumnShowList.includes(o.__config__.tsaDK) || o.isStorage);
    let searchOptions = list.filter(o => !noSearchList.includes(o.__config__.tsaDK));
    if (unref(webType) == 4) {
      const interfaceParam = (props.interfaceParam || [])
        .filter(o => o.useSearch)
        .map(o => {
          let tsaDK = 'input';
          if (o.dataType === 'int' || o.dataType === 'decimal') tsaDK = 'inputNumber';
          if (o.dataType === 'datetime') tsaDK = 'datePicker';
          return {
            id: o.field,
            fullName: o.fieldName,
            __vModel__: o.field,
            __config__: { isFromParam: true, tsaDK },
          };
        });
    ...
    state.groupFieldOptions = list.filter(o => o.id.indexOf('-') < 0 && !noGroupList.includes(o.__config__.tsaDK)).map(o => ({ ...o, disabled: false }));
    state.treeFieldOptions = list.filter(o => o.id.indexOf('-') < 0 && o.__config__.tsaDK == 'treeSelect');
    state.columnOptions = columnOptions.map(o => ({
      label: o.fullName,
      prop: o.id,
      fixed: 'none',
      align: 'left',
      tsaDK: o.__config__.tsaDK,
      sortable: false,
      width: null,
      ...o,
    }));
    state.searchOptions = searchOptions.map(o => ({
      ...
      searchMultiple: getSearchMultiple(o.__config__.tsaDK),
      isKeyword: false,
      ...o,
    }));
