<template>
  <BasicModal v-bind="$attrs" @register="registerModal" title="组件属性配置" @ok="handleSubmit" destroyOnClose class="extra-config-modal">
    <div class="extra-config-modal-body" :style="{ 'min-height': activeData.tsaDK === 'select' ? '300px' : '150px' }">
      <a-form :colon="false" labelAlign="left" :labelCol="{ style: { width: '90px' } }" class="right-board-form">
        <template v-if="activeData.tsaDK === 'select'">
          ...
          <div class="add-btn" v-if="activeData.__config__.dataType === 'static'">
            <a-button type="link" preIcon="icon-bts icon-bts-btn-add" @click="addSelectItem" class="!px-0">添加选项</a-button>
          ...
          <div v-if="activeData.__config__.dataType === 'dictionary'">
            ...
            <a-form-item label="存储字段">
              <tsa-select v-model:value="activeData.props.value" placeholder="请选择" :options="valueOptions" />
            </a-form-item>
          </div>
        ...
        <a-form-item label="日期类型" v-if="activeData.tsaDK === 'datePicker'">
          <tsa-select v-model:value="activeData.format" placeholder="请选择" :options="dateFormatOptions" />
        </a-form-item>
        <a-form-item label="时间类型" v-if="activeData.tsaDK === 'timePicker'">
          <tsa-select v-model:value="activeData.format" placeholder="请选择" :options="timeFormatOptions" />
        </a-form-item>
        <a-form-item label="" v-if="['organizeSelect', 'depSelect', 'userSelect'].includes(activeData.tsaDK)">
          <a-checkbox v-model:checked="activeData.isIncludeSubordinate">
            查询当前{{ activeData.tsaDK === 'organizeSelect' ? '组织及子组织' : activeData.tsaDK === 'depSelect' ? '部门及子部门' : '用户及下属' }}
          </a-checkbox>
        </a-form-item>
      </a-form>
