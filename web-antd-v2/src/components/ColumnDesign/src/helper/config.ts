...
const getSearchType = item => {
  ...
  if (RangeList.includes(tsaDK)) return 3;
  if (fuzzyList.includes(tsaDK)) return 2;
  return 1;
};
const getSearchMultiple = tsaDK => {
  const searchMultipleList = ['select', 'depSelect', 'roleSelect', 'userSelect', 'usersSelect', 'organizeSelect', 'posSelect', 'groupSelect'];
  return searchMultipleList.includes(tsaDK);
};
...
const defaultColumnData = {
  ...
  customBtnsList: [],
  btnsList: [{ value: 'add', icon: 'icon-bts icon-bts-btn-add', label: '新增' }], // 按钮
  columnBtnsList: [
