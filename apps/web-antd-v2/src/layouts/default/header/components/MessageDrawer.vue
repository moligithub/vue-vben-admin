    ...
    <ScrollContainer ref="infiniteBody" class="msg-list">
      <div v-for="(item, i) in list" :key="i" class="msg-list-item" :title="item.title" @click="readInfo(item)" v-if="list.length">
        <Badge :dot="item.isRead != '1'" status="warning">
          <i class="icon-bts icon-bts-xitong item-icon" v-if="item.type == 1"></i>
          <i class="icon-bts icon-bts-generator-notice item-icon notice-icon" v-else-if="item.type == 3"></i>
          <i class="icon-bts icon-bts-portal-schedule item-icon schedule-icon" v-else-if="item.type == 4"></i>
          <i class="icon-bts icon-bts icon-bts-generator-flow item-icon flow-icon" v-else></i>
        </Badge>
...
<script lang="ts" setup>
  import { getMessageList, readAllMsg, readInfo as readMsgInfo } from '/@/api/systemmgr/message';
  ...
  import { toDateValue } from '/@/utils/tsa';
  import { encryptByBase64 } from '/@/utils/cipher';
  import Detail from '/@/views/systemmgr/notice/Detail.vue';
  import { useBaseStore } from '/@/store/modules/base';
  import ScheduleDetail from '/@/components/VisualPortal/Portal/HSchedule/Detail.vue';
  import { getScheduleDetail } from '/@/api/lowcodedev/portal';
