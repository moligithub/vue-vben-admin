<template>
  <BasicModal v-bind="$attrs" @register="registerModal" :title="getTitle" showOkBtn @ok="handleSubmit" class="tsa-condition-modal" destroy-on-close>
    <a-form :colon="false" :labelCol="{ style: { width: '0' } }" :model="dataForm" :rules="dataRule" ref="formElRef" class="condition-main">
      ...
      <div class="mb-10px" v-if="condition.length">
        ...
        <tsa-select v-model:value="dataForm.matchLogic" placeholder="请选择" :options="logicOptions" class="!w-68px" />
      </div>
      <a-form-item v-for="(item, index) in condition" :key="index" class="!mb-10px">
        <div>
          <a-row :gutter="8">
            <a-col :span="6">
              <tsa-select v-model:value="item.logic" placeholder="请选择" :options="logicOptions" />
            </a-col>
            <a-col :span="5">
              <a-button preIcon="icon-bts icon-bts-btn-add" class="w-full" @click="addItem(index)">添加条件</a-button>
            </a-col>
            <a-col :span="5">
          ...
          <a-row :gutter="8" v-for="(child, childIndex) in item.groups" :key="index + childIndex" class="mt-10px">
            <a-col :span="6">
              <tsa-select v-model:value="child.id" placeholder="请选择字段" allowClear :options="fieldOptions" @change="onFieldChange($event, child)" />
            </a-col>
            <a-col :span="5">
              <tsa-select v-model:value="child.op" placeholder="选择符号" allowClear :options="child.opOptions" @change="onSymbolChange($event, child)" />
            </a-col>
            <a-col :span="12">
              <template v-if="child.conditionText === 'inputNumber'">
                <tsa-number-range v-model:value="child.value" :disabled="child.disabled" v-if="child.op == 'between'" />
                <tsa-input-number v-model:value="child.value" :disabled="child.disabled" placeholder="请输入" v-else />
              </template>
      ...
      <a-form-item class="!mb-0">
        <span class="link-text inline-block" @click="addGroup()"><i class="icon-bts icon-bts-btn-add text-14px mr-4px"></i>添加分组</span>
      </a-form-item>
    </a-form>
  </BasicModal>
</template>
<script lang="ts" setup>
  ...
  import { createScheme, updateScheme, getSchemeInfo, getDataAuthorizeList } from '/@/api/systemmgr/dataAuthorize';
    ...
    getDataAuthorizeList({ moduleId: state.dataForm.moduleId })
      .then(res => {
        ...
        state.fieldOptions.map(o => {
          o.enCode = o.enCode.replace('_tsa_', '.');
          o.enCode = o.enCode.replace('tsa_', '');
        });
