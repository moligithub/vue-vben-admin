<template>
  <BasicPopup v-bind="$attrs" @register="registerPopup" :title="title" class="full-popup" destroy-on-close>
    ...
    <a-tabs v-model:activeKey="listQuery.platform" type="card" class="tsa-content-table-tabs" destroyInactiveTabPane>
      <a-tab-pane key="Web" tab="Web门户">
        <BasicTable @register="registerWebTable" :columns="menuTableColumns" :searchInfo="getSearchInfo">
          <template #tableTitle>
            <a-button type="primary" preIcon="icon-bts icon-bts-btn-add" @click="addOrUpdateHandle()">添加</a-button>
          </template>
...
<script lang="ts" setup>
  ...
  import { getPortalManageList, delPortalManage } from '/@/api/lowcodedev/portal';
