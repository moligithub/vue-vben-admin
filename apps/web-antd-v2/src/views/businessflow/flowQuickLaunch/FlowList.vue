<template>
  <div class="flow-list-container">
              ...
              <a-col :span="6" v-for="(item, i) in list" :key="i" class="item" @click="handleClick(item)">
                <a-card hoverable>
                  <div class="item-icon" :style="{ backgroundColor: item.iconBackground || '#008cff' }">
                    <i :class="item.icon || 'icon-bts icon-bts-funcFlow'"></i>
                  </div>
    ...
    <BasicModal
      ...
      @ok="selectUser"
      destroyOnClose
      class="tsa-flow-user-list-modal">
...
<script lang="ts" setup>
  ...
  import { getFlowEnginePageList, getFlowList } from '/@/api/businessflow/flowEngine';
  import { getDelegateFlow, getDelegateUserByFlowId } from '/@/api/businessflow/flowDelegate';
