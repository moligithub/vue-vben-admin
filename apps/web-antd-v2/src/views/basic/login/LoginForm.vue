<template>
  ...
  <BasicModal v-bind="$attrs" @register="registerSsoModal" title="登录" :footer="null" :width="1000" class="tsa-sso-modal" :closeFunc="onSsoModalClose">
    <iframe width="100%" height="100%" :src="ssoUrl" frameborder="0" v-if="showIframe"></iframe>
  </BasicModal>
  <BasicModal v-bind="$attrs" @register="registerTenantSocialModal" :closable="false" :footer="null" :width="700" class="tsa-tenant-social-modal">
    <div class="other-main">
      <div class="other-title">
        <div class="other-icon"><i class="icon-bts icon-bts-user" /></div>
...
<script lang="ts" setup>
  ...
  async function handleLogin() {
    ...
    try {
      loading.value = true;
      console.log("orig password:"+data.password)
      ...
      console.log("AES password:"+encryptPassword)
    ...
    getTicket().then(res => {
      ...
      state.socialsList.forEach(item => {
        if (enname == item.enname) {
          const renderUrl = item.renderUrl.replace('TSA_TICKET', state.ssoTicket);
  ...
  function handleChangeImg() {
    ...
    state.codeImgUrl = `/bot/authsession/ImageCode/${state.codeLength || 4}/${timestamp}`;
  }
