<template>
  <div class="tsa-content-wrapper short-link-wrapper short-link-wrapper-list">
    ...
    <div class="short-link-main" v-show="!columnPassUse">
      <a-popover placement="bottomRight">
        ...
        <i class="bts-custom bts-custom-qrcode icon-qrcode"></i>
      </a-popover>
            ...
            <template #bodyCell="{ column, record }">
              ...
              <template v-if="!(record.top || column.id?.includes('-'))">
                <template v-if="column.tsaDK === 'inputNumber'">
                  <tsa-input-number v-model:value="record[column.dataIndex]" :precision="column.precision" :thousands="column.thousands" disabled detailed />
                </template>
...
<script lang="ts" setup>
  import { getModelList, getConfig, checkPwd } from '/@/api/lowcodedev/shortLink';
  ...
  import { getScriptFunc, thousandsFormat } from '/@/utils/tsa';
  ...
  function validFields(o) {
    if (!o.__config__ || !o.__config__.tsaDK) return true;
    const tsaDK = o.__config__.tsaDK;
    if (validFieldsList.includes(tsaDK) || (selectFieldsList.includes(tsaDK) && o.__config__.dataType === 'static')) return true;
    return false;
  }
