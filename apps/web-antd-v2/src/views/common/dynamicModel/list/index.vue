<template>
  <div class="tsa-content-wrapper">
    <div class="tsa-content-wrapper-left" v-if="columnData.type === 2">
    ...
    <div class="tsa-content-wrapper-center">
      <div class="tsa-content-wrapper-search-box" v-if="columnData.searchList?.length">
        <BasicForm
      ...
      <div class="tsa-content-wrapper-content">
              ...
              <a-tab-pane :key="cIndex" :tab="child.label" :label="child.label" v-for="(child, cIndex) in childColumnList">
                <BasicTable @register="registerChildTable" :ellipsis="!!columnData.showOverflow" :data-source="record[child.prop]" :columns="child.children">
                  <template #bodyCell="{ column, record: childRecord }">
                    ...
                    <template v-if="column.tsaDK === 'calculate'">
                      <tsa-calculate
                        v-model:value="childRecord[column.dataIndex]"
                    ...
                    <template v-if="column.tsaDK === 'sign'">
                      <tsa-sign v-model:value="childRecord[column.dataIndex]" detailed />
                    </template>
                    <template v-if="column.tsaDK === 'signature'">
                      <tsa-signature v-model:value="childRecord[column.dataIndex]" detailed />
                    </template>
                    <template v-if="column.tsaDK === 'rate'">
                      <tsa-rate v-model:value="childRecord[column.dataIndex]" :count="column.count" :allowHalf="column.allowHalf" disabled />
                    </template>
                    <template v-if="column.tsaDK === 'slider'">
                      <tsa-slider v-model:value="childRecord[column.dataIndex]" :min="column.min" :max="column.max" :step="column.step" disabled />
                    </template>
                    <template v-if="column.tsaDK === 'uploadImg'">
                      <tsa-upload-img v-model:value="childRecord[column.dataIndex]" disabled detailed simple v-if="childRecord[column.dataIndex]?.length" />
                    </template>
                    <template v-if="column.tsaDK === 'uploadFile'">
                      <tsa-upload-file v-model:value="childRecord[column.dataIndex]" disabled detailed simple v-if="childRecord[column.dataIndex]?.length" />
                    </template>
                    <template v-if="column.tsaDK === 'input'">
                      <tsa-input
                        v-model:value="childRecord[column.dataIndex]"
          ...
          <template #bodyCell="{ column, record, index }">
            <template v-if="column.flag === 'INDEX'">
              <div class="edit-row-action" v-if="columnData.type === 4 && !config.enableFlow">
                ...
                <i class="bts-custom bts-custom-arrow-expand" @click="handleRowForm(record)"></i>
              </div>
            ...
            <template v-if="columnData.type === 4">
              <template v-if="record.rowEdit">
                ...
                <template v-else-if="column.tsaDK === 'calculate'">
                  <tsa-calculate
                    v-model:value="record[column.prop]"
                    :isStorage="column.isStorage"
                    :precision="column.precision"
                ...
                <template v-else-if="column.tsaDK === 'rate'">
                  <tsa-rate v-model:value="record[column.prop]" :count="column.count" :allowHalf="column.allowHalf" :disabled="column.disabled" />
                </template>
                <template v-else-if="column.tsaDK === 'slider'">
                  <tsa-slider v-model:value="record[column.prop]" :min="column.min" :max="column.max" :step="column.step" :disabled="column.disabled" />
                </template>
                <template v-else-if="column.tsaDK === 'uploadImg'">
                  <tsa-upload-img
                    v-model:value="record[column.prop]"
                ...
                <template v-else-if="column.tsaDK === 'uploadFile'">
                  <tsa-upload-file
                    v-model:value="record[column.prop]"
                ...
                <template v-else-if="column.tsaDK === 'switch'">
                  <tsa-switch v-model:value="record[column.prop]" :disabled="column.disabled" />
                </template>
                <template v-else-if="column.tsaDK === 'timePicker'">
                  <tsa-time-picker
                    v-model:value="record[column.prop]"
                ...
                <template v-else-if="column.tsaDK === 'datePicker'">
                  <tsa-date-picker
                    v-model:value="record[column.prop]"
                    :format="column.format"
                    :startTime="column.startTime"
                ...
                <template v-else-if="column.tsaDK === 'organizeSelect'">
                  <tsa-organize-select
                    v-model:value="record[column.prop]"
                    :placeholder="column.placeholder"
                ...
                <template v-else-if="column.tsaDK === 'depSelect'">
                  <tsa-dep-select
                    v-model:value="record[column.prop]"
                    :placeholder="column.placeholder"
                    :multiple="column.multiple"
                ...
                <template v-else-if="column.tsaDK === 'roleSelect'">
                  <tsa-role-select
                    v-model:value="record[column.prop]"
                    :placeholder="column.placeholder"
                    :multiple="column.multiple"
                ...
                <template v-else-if="column.tsaDK === 'groupSelect'">
                  <tsa-group-select
                    v-model:value="record[column.prop]"
                    :placeholder="column.placeholder"
                    :multiple="column.multiple"
                ...
                <template v-else-if="column.tsaDK === 'posSelect'">
                  <tsa-pos-select
                    v-model:value="record[column.prop]"
                    :placeholder="column.placeholder"
                    :multiple="column.multiple"
                ...
                <template v-else-if="column.tsaDK === 'userSelect'">
                  <tsa-user-select
                    v-model:value="record[column.prop]"
                    :placeholder="column.placeholder"
                    :multiple="column.multiple"
                ...
                <template v-else-if="column.tsaDK === 'usersSelect'">
                  <tsa-users-select
                    v-model:value="record[column.prop]"
                    :placeholder="column.placeholder"
                    :multiple="column.multiple"
                ...
                <template v-else-if="column.tsaDK === 'areaSelect'">
                  <tsa-area-select
                    v-model:value="record[column.prop]"
                    :level="column.level"
                    :placeholder="column.placeholder"
                ...
                <template v-else-if="['select', 'radio', 'checkbox'].includes(column.tsaDK)">
                  <tsa-select
                    v-model:value="record[column.prop]"
                    :placeholder="column.placeholder"
                    :multiple="column.multiple || column.tsaDK === 'checkbox'"
                    :allowClear="column.clearable || ['radio', 'checkbox'].includes(column.tsaDK)"
                    :showSearch="column.filterable"
                    :disabled="column.disabled"
                    :options="column.options"
                    :fieldNames="column.props" />
                </template>
                <template v-else-if="column.tsaDK === 'cascader'">
                  <tsa-cascader
                    v-model:value="record[column.prop]"
                    :placeholder="column.placeholder"
                    :multiple="column.multiple"
                ...
                <template v-else-if="column.tsaDK === 'treeSelect'">
                  <tsa-tree-select
                    v-model:value="record[column.prop]"
                    :placeholder="column.placeholder"
                    :multiple="column.multiple"
                ...
                <template v-else-if="column.tsaDK === 'relationForm'">
                  <tsa-relation-form
                    v-model:value="record[column.prop]"
                    :placeholder="column.placeholder"
                    :allowClear="column.clearable"
                ...
                <template v-else-if="column.tsaDK === 'popupSelect' || column.tsaDK === 'popupTableSelect'">
                  <tsa-popup-select
                    v-model:value="record[column.prop]"
                    :placeholder="column.placeholder"
                    :multiple="column.multiple"
                ...
                <template v-else-if="column.tsaDK === 'autoComplete'">
                  <tsa-auto-complete
                    v-model:value="record[column.prop]"
                    :placeholder="column.placeholder"
                    :allowClear="column.clearable"
                ...
                <template v-else-if="['input', 'textarea'].includes(column.tsaDK)">
                  <tsa-input
                    v-model:value="record[column.prop]"
                    :placeholder="column.placeholder"
                    :allowClear="column.clearable"
                ...
                <template v-else-if="column.tsaDK === 'location'">
                  <tsa-location
                    v-model:value="record[column.prop]"
                ...
                <template v-else-if="column.tsaDK === 'sign'">
                  <tsa-sign v-model:value="record[column.prop]" :disabled="column.disabled" />
                </template>
                <template v-else-if="column.tsaDK === 'signature'">
                  <tsa-signature v-model:value="record[column.prop]" :disabled="column.disabled" />
                </template>
                <template v-else-if="systemComponentsList.includes(column.tsaDK)">
              ...
              <template v-else>
                ...
                <template v-else-if="column.tsaDK === 'sign'">
                  <tsa-sign v-model:value="record[column.prop + '_name']" detailed />
                </template>
                <template v-else-if="column.tsaDK === 'signature'">
                  <tsa-signature v-model:value="record[column.prop + '_name']" detailed />
                </template>
                <template v-else-if="column.tsaDK === 'rate'">
                  <tsa-rate v-model:value="record[column.prop + '_name']" :count="column.count" :allowHalf="column.allowHalf" disabled />
                </template>
                <template v-else-if="column.tsaDK === 'slider'">
                  <tsa-slider v-model:value="record[column.prop + '_name']" :min="column.min" :max="column.max" :step="column.step" disabled />
                </template>
                <template v-else-if="column.tsaDK === 'uploadImg'">
                  <tsa-upload-img v-model:value="record[column.prop + '_name']" disabled detailed simple v-if="record[column.prop]?.length" />
                </template>
                <template v-else-if="column.tsaDK === 'uploadFile'">
                  <tsa-upload-file v-model:value="record[column.prop + '_name']" disabled detailed simple v-if="record[column.prop]?.length" />
                </template>
                <template v-else-if="column.tsaDK === 'input'">
                  <tsa-input
                    v-model:value="record[column.prop + '_name']"
                    :useMask="column.useMask"
                    :maskConfig="column.maskConfig"
            ...
            <template v-else>
              ...
              <template v-if="!(record.top || column.id?.includes('-'))">
                ...
                <template v-if="column.tsaDK === 'inputNumber'">
                  <tsa-input-number v-model:value="record[column.prop]" :precision="column.precision" :thousands="column.thousands" disabled detailed />
                </template>
                <template v-if="column.tsaDK === 'calculate'">
                  <tsa-calculate
                    v-model:value="record[column.prop]"
                ...
                <template v-if="column.tsaDK === 'sign'">
                  <tsa-sign v-model:value="record[column.prop]" detailed />
                </template>
                <template v-if="column.tsaDK === 'signature'">
                  <tsa-signature v-model:value="record[column.prop]" detailed />
                </template>
                <template v-if="column.tsaDK === 'rate'">
                  <tsa-rate v-model:value="record[column.prop]" :count="column.count" :allowHalf="column.allowHalf" disabled />
                </template>
                <template v-if="column.tsaDK === 'slider'">
                  <tsa-slider v-model:value="record[column.prop]" :min="column.min" :max="column.max" :step="column.step" disabled />
                </template>
                <template v-if="column.tsaDK === 'uploadImg'">
                  <tsa-upload-img v-model:value="record[column.prop]" disabled detailed simple v-if="record[column.prop]?.length" />
                </template>
                <template v-if="column.tsaDK === 'uploadFile'">
                  <tsa-upload-file v-model:value="record[column.prop]" disabled detailed simple v-if="record[column.prop]?.length" />
                </template>
                <template v-if="column.tsaDK === 'input'">
                  <tsa-input
                    v-model:value="record[column.prop]"
...
<script lang="ts" setup>
  import { getModelList, exportModel, delModel, batchDelete, getConfigData, createModel, updateModel, getModelInfo } from '/@/api/lowcodedev/visualDev';
  import { create as createFlowForm, update as updateFlowForm } from '/@/api/businessflow/workFlowForm';
  import { getCandidates } from '/@/api/businessflow/flowBefore';
  import { getDataInterfaceRes } from '/@/api/systemdata/dataInterface';
  import { getOrgByOrganizeCondition } from '/@/api/permissionmgr/organize';
  import { ref, reactive, onMounted, toRefs, computed, unref, nextTick, toRaw, provide } from 'vue';
  import { getFlowList } from '/@/api/businessflow/flowEngine';
  ...
  import FlowParser from '/@/views/businessflow/components/FlowParser.vue';
  import CandidateModal from '/@/views/businessflow/components/modal/CandidateModal.vue';
  import { ExportModal, ImportModal, SuperQueryModal, SelectFlowModal } from '/@/components/CommonModal';
  import { downloadByUrl } from '/@/utils/file/download';
  import { useRoute } from 'vue-router';
  import { getScriptFunc, onlineUtils, getDateTimeUnit, getTimeUnit, thousandsFormat, getParamList } from '/@/utils/tsa';
  ...
  const getSummaryColumn = computed(() => {
    ...
    for (let i = 0; i < defaultColumns.length; i++) {
      ...
      if (e.tsaDK === 'table' || e.tsaDK === 'complexHeader') {
  ...
  function handleLeftTreeSelect(id, _node, nodePath) {
    ...
    if (state.treeRelationObj?.tsaDK && ['organizeSelect', 'cascader', 'areaSelect'].includes(state.treeRelationObj.tsaDK)) {
      let currValue = [];
      if (state.columnData.treeDataSource === 'formField' && state.treeRelationObj.tsaDK === 'organizeSelect') {
  ...
  function addHandleForRowEdit() {
    let item = { rowEdit: true, id: 'tsaAdd' };
  ...
  function cancelRowEdit(record, index) {
    const id = !record.id || record.id === 'tsaAdd' ? '' : record.id;
    if (!id) return deleteTableDataRecord('tsaAdd');
  ...
  function submitForRowEdit(record) {
    if (props.isPreview) return createMessage.warning('功能预览不支持数据保存');
    record.id = !record.id || record.id === 'tsaAdd' ? '' : record.id;
  ...
  async function getTreeView(isInit = false) {
    ...
    if (state.columnData.treeDataSource === 'formField') {
      ...
      const tsaDK = treeRelationObj?.__config__?.tsaDK || '';
      if (state.columnData.treeRelation && ['organizeSelect', 'depSelect'].includes(tsaDK)) {
        ...
        if (treeRelationObj.selectType === 'custom' && treeRelationObj.ableIds?.length) {
          const departIds = tsaDK === 'organizeSelect' ? treeRelationObj.ableIds.map(o => o[o.length - 1]) : treeRelationObj.ableIds;
    ...
    if (state.columnData.treeRelation) {
      for (let i = 0; i < state.columnData.columnOptions.length; i++) {
        ...
        if (e.id === state.columnData.treeRelation) {
          state.treeRelationObj = { ...e, searchMultiple: false, tsaDK: e.__config__.tsaDK };
          break;
        }
      }
    }
  ...
  function getChildComplexColumns(columnList) {
    ...
    state.childColumnList = list.filter(o => o.tsaDK === 'table');
    // 子表分组展示宽度取100
  ...
  function getMergeList(list) {
    list.forEach(item => {
      if (item.tsaDK === 'table' && item.children && item.children.length) {
