        ...
        <a-form :colon="false" :labelCol="{ style: { width: '110px' } }">
          <a-form-item label="用户邮箱">
            <a-col v-for="(item, i) in emailList" :key="i" class="align-center" :class="{ 'mt-10px': i != 0 }">
              <tsa-select v-model:value="item.email" :options="options" showSearch placeholder="选择邮箱" />
              <a-input v-model:value="item.emailText" placeholder="" />
              <a-button type="danger" @click="handleDelete(i)" v-if="i != 0"><i class="icon-bts icon-bts-nav-close" /></a-button>
              <a-button type="primary" @click="handleAdd" style="" v-if="i == 0"><i class="icon-bts icon-bts-btn-add" /></a-button>
            </a-col>
          </a-form-item>
          ...
          <a-form-item label="产品评价">
            <tsa-color-picker v-model:value="color" />
          </a-form-item>
          <a-form-item label="产品评价">
            <tsa-slider v-model:value="range" />
          </a-form-item>
          ...
          <TsaGroupTitle content="订单详细" :bordered="false" />
...
<script lang="ts" setup>
  ...
  import GoodsModal from '/@/views/businessflow/workFlowForm/crmOrder/GoodsModal.vue';
