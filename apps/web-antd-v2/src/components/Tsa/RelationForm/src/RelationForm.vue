...
<script lang="ts" setup>
  import { getFieldDataSelect, getDataChange, getConfigData } from '/@/api/lowcodedev/visualDev';
  ...
  defineOptions({ name: 'TsaRelationForm', inheritAttrs: false });
  ...
  function setValue(value) {
    ...
    if (!props.multiple) {
      getDataChange(props.modelId, value).then(res => {
        ...
        emitter.emit('setRelationData', { tsaRelationField: props.field, ...data, id: res.data.id });
      });
    }
  }
  function setNullValue() {
    ...
    generatorStore.setRelationData(relationData);
    emitter.emit('setRelationData', { tsaRelationField: props.field });
  }
  function onChange() {
    options.value = [];
    emit('update:value', '');
