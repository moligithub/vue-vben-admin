<template>
  <a-modal v-model:visible="visible" title="选择签章" class="tsa-signature-modal" centered :maskClosable="false" :keyboard="false" :width="600">
    <template #closeIcon>
    ...
    <div class="sign-main">
      <a-form :colon="false" :labelCol="{ style: { width: '80px' } }" :model="dataForm" ref="formElRef">
        <a-form-item label="电子签章" name="innerValue">
          <tsa-select v-model:value="dataForm.innerValue" :options="options" showSearch allowClear />
        </a-form-item>
      </a-form>
    </div>
  </a-modal>
...
<script lang="ts" setup>
  import { getListByIds } from '/@/api/systemmgr/signature';
