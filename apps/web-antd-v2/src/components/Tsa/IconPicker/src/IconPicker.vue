          ...
          <a-tab-pane key="1" tab="公共图标">
            <ScrollContainer>
              <div class="icon-box-list">
                <a-button v-for="(item, index) in bstIconList" :key="index" @click="handleIconClick(item)" :class="{ 'is-active': item === active }">
                  <i :class="item" />
                </a-button>
              </div>
            </ScrollContainer>
          </a-tab-pane>
          <a-tab-pane key="2" tab="自定义图标">
            <ScrollContainer>
              <div class="icon-box-list">
                <a-button v-for="(item, index) in bstCustomList" :key="index" @click="handleIconClick(item)" :class="{ 'is-active': item === innerValue }">
                  <i :class="item" />
                </a-button>
              </div>
            </ScrollContainer>
...
<script lang="ts" setup>
  ...
  import { bstIconJson } from '../data/bstIcon';
  import { bstCustomJson } from '../data/bstCustom';
  import { cloneDeep } from 'lodash-es';
  import { useI18n } from '/@/hooks/web/useI18n';

  const bstIcon = bstIconJson.glyphs.map(o => `icon-bts icon-bts-${o.font_class}`);
  const bstCustom = bstCustomJson.glyphs.map(o => `bts-custom bts-custom-${o.font_class}`);

  defineOptions({ name: 'TsaIconPicker', inheritAttrs: false });
  ...
  const { keyword, activeKey, bstIconList, bstCustomList } = toRefs(state);
