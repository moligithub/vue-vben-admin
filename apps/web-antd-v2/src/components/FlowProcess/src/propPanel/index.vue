...
<script lang="ts" setup>
  ...
  import { getInfo as getFormInfo } from '/@/api/businessflow/formDesign';
  ...
  // 所有表单字段
  const formFieldsOptions = computed(() => state.formFieldList.filter(o => o.__config__.tsaDK !== 'table').map(o => ({ ...o, disabled: false })));
  // 不包含子表的表单字段
  ...
  const requiredDisabled = tsaDK => {
    return ['billRule', 'createUser', 'createTime', 'modifyTime', 'modifyUser', 'currPosition', 'currOrganize', 'table'].includes(tsaDK);
  };
  const getDataType = data => {
    if (!data.__config__ || !data.__config__.tsaDK) return '';
    const tsaDK = data.__config__.tsaDK;
    if (['inputNumber', 'datePicker', 'rate', 'slider'].includes(tsaDK)) {
      return 'number';
    } else if (['checkbox', 'uploadFile', 'uploadImg', 'cascader', 'organizeSelect', 'areaSelect'].includes(tsaDK)) {
      return 'array';
    } else if (
    ...
    if (!formOperates.length || isUpdate) {
      for (let i = 0; i < state.formFieldList.length; i++) {
        ...
        res.push({
          ...
          requiredDisabled: requiredDisabled(data.__config__.tsaDK) || data.__config__.required,
          tsaDK: data.__config__.tsaDK,
  ...
  function getConditionNodeFieldList() {
    ...
    if (!state.prevNodeList.length) {
      ...
      state.formFieldList = formFieldList.filter(o => o.__config__.tsaDK !== 'table');
    }
  }
  // 获取上一节数据
