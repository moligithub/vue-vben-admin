<template>
  <BasicModal v-bind="$attrs" @register="registerModal" title="请求接口" :closeFunc="onClose" :footer="null" :width="600" class="webhook-request-modal">
    <div class="main">
      ...
      <a-input :value="getSysConfig.tsaDomain + state.url" readonly>
        <template #addonAfter>
          <span class="cursor-pointer" @click="handleCopy(getSysConfig.tsaDomain + state.url)">复制链接</span>
        </template>
      </a-input>
      <div class="mt-20px">
...
<script lang="ts" setup>
  ...
  import { getWebhookParams, webhookStart } from '/@/api/lowcodedev/integrate';
