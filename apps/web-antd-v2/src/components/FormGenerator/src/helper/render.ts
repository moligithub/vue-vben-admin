...
export default defineComponent({
  ...
  setup(props, { emit, expose }) {
    ...
    function buildDataObject(confClone, dataObject, formData) {
      const tsaDK = confClone.__config__.tsaDK;
      ...
      if (['calculate', 'table', 'barcode', 'qrcode', 'popupSelect', 'popupTableSelect', 'autoComplete'].includes(tsaDK)) {
        dataObject['formData'] = formData;
      }
      if (['table'].includes(tsaDK)) {
        dataObject['relations'] = props.relations;
        dataObject['vModel'] = confClone.__vModel__;
        dataObject['ref'] = 'tableRef';
      }
      if (['relationForm', 'popupSelect'].includes(tsaDK)) {
        dataObject['field'] = confClone.__config__.tableName
          ? confClone.__vModel__ + '_tsaTable_' + confClone.__config__.tableName + (confClone.__config__.isSubTable ? '0' : '1')
    ...
    return () => {
      ...
      const tsaDK = upperFirst(props.conf.__config__.tsaDK) === 'Table' ? 'InputTable' : upperFirst(props.conf.__config__.tsaDK);
      const Comp = componentMap.get(tsaDK as ComponentType) as ReturnType<typeof defineComponent>;
      if (!Comp) return null;
      const realDataObject = getRealProps(dataObject, props.conf.__config__.tsaDK);
