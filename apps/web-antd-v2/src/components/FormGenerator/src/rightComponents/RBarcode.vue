<template>
  <a-form-item label="编码格式">
    <tsa-select v-model:value="activeData.format" :options="barcodeFormatOptions" />
  </a-form-item>
  <a-form-item label="条码颜色">
    <tsa-color-picker v-model:value="activeData.lineColor" size="small" />
  </a-form-item>
  ...
  <a-form-item label="默认值">
    <tsa-select v-model:value="activeData.dataType" :options="dataTypeOptions" />
  </a-form-item>
  <a-form-item label="固定值" v-if="activeData.dataType === 'static'">
    <tsa-input v-model:value="activeData.staticText" placeholder="请输入" allowClear @change="onTextChange" />
  </a-form-item>
  <a-form-item label="选择组件" v-if="activeData.dataType === 'relation'">
    <tsa-select v-model:value="activeData.relationField" :options="drawingOptions" allowClear showSearch />
  </a-form-item>
</template>
