<template>
  ...
  <a-form-item label=" " :labelCol="{ style: { width: '30px' } }">
    <tsa-radio v-model:value="activeData.__config__.dataType" :options="dataTypeOptions" optionType="button" buttonStyle="solid" @change="onDataTypeChange" />
  </a-form-item>
  <div class="options-list" v-if="activeData.__config__.dataType === 'static'">
    <template v-if="['treeSelect', 'cascader'].includes(activeData.__config__.tsaDK)">
      <BasicTree ref="treeRef" :treeData="activeData.options" :actionList="actionList" />
      <div class="add-btn">
        <a-button type="link" preIcon="icon-bts icon-bts-btn-add" @click="addTreeItem" class="!px-0">添加父级</a-button>
  ...
  <div v-if="activeData.__config__.dataType === 'dictionary'">
    <a-form-item label="数据字典">
      <tsa-tree-select :options="dicOptions" v-model:value="activeData.__config__.dictionaryType" lastLevel allowClear @change="onDictionaryTypeChange" />
    </a-form-item>
  ...
  <div v-if="activeData.__config__.dataType === 'dynamic'">
    ...
    <a-form-item label="子级字段" v-if="['treeSelect', 'cascader'].includes(activeData.__config__.tsaDK)">
